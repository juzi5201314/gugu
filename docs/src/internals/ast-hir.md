...
`EffectSet` 只有 8 个编译器布尔标志，使用零分配 `u32` 位掩码，位固定表示 `MAY_PANIC`、`MAY_ALLOCATE`、`MAY_SAFEPOINT`、`MAY_SUSPEND`、`READS_MEMORY`、`WRITES_MEMORY`、`FOREIGN_CALL` 和 `UNSAFE_OPERATION`。构造/合并后以 `debug_assert!(bits & !KNOWN_EFFECT_BITS == 0)` 检查上界。该集合供 GIR 构造和优化验证使用，不是用户可观察的效果类型系统。
