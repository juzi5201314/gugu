...
| 30 | `RuntimeRawModel` | target 语义 + tuning profile revision + LIR 指纹 + placement 指纹 + 平台 profile + 资源需求视图（schema 13，域 `gugu-runtime-raw-v1`） | `RuntimeRawContractV1`（dense size class、消息字段 schema、grace 步骤，以及 ResourceCell header/状态位/release 描述符/资源种类目录、`PlatformRangeSchemaV1` 操作目录与失败映射、`LedgerSchemaV1` 互斥分类、`TurnRegionRuntimeContract` 的容量 class/对象上界/export 位/状态/`RegionTransfer` 字段目录与需求） |
