...
| 11 | `BuildGenericGir` | 冻结 HIR 指纹 + owner 稳定键 + owner 下标（schema 5，域 `gugu-build-generic-gir-v1`） | generic GIR body（含 `large_copies`） |
...
| 30 | `RuntimeRawModel` | target 语义 + tuning profile revision + LIR 指纹 + placement 指纹 + 平台 profile + 资源需求视图（schema 26，域 `gugu-runtime-raw-v1`） | `RuntimeRawContractV1`（dense size class、消息字段 schema、grace 步骤，以及 ResourceCell header/状态位/release 描述符/资源种类目录、`PlatformRangeSchemaV1` 操作目录与失败映射、`LedgerSchemaV1` 互斥分类、`BarrierRuntimeContract` 的 hybrid 序列/card 缓存与 edge scratch 预留、`TurnRegionRuntimeContract` 的容量 class/对象上界/export 位/状态/`RegionTransfer` 字段目录与需求、`LocalHeapRuntimeContract` 的 Immix/TLAB 尺寸、`HeapBlockRecord` 布局与分代 cycle 参数、`MarkRuntimeContract` 的 mailbox/credit 目录、七项收敛条件来源绑定与 `MarkTicket` 字段集合与 record 布局、`EdgeRuntimeContract` 的候选相位/block 状态/`EdgeDelta` 字段集合与 record 布局、`BlockReturnRuntimeContract` 的 return unit/发布门禁/queue-page grace、`CompressionRuntimeContract` 的 cage 位布局/上限与粒度/FFI 交接规则/统计名/目标能力/`CompressionDemand`、`RoutingRuntimeContract` 的路由模式/固定 `2^k` bucket/有限 levels/hop 上限/maintenance 相位序列/`RoutingDemand`、`ProvenanceRuntimeContract` 的释放 provenance 检查目录/per-domain secret 登记与管理规则/release-debug-security 检查绑定/拒绝分类/统计口径/`ProvenanceDemand`、`CombiningRuntimeContract` 的冷操作 tag 目录/允许与禁止用途/operation record 字段集合/状态与迁移/同步路径/合并与轮次预算/`CombiningDemand`、`BridgeContract` 的阻塞额度/dirty 槽/三种模式与 `gugu-bridge-v1` 指纹） |
...
`BuildGenericGir` 为每个冻结 HIR owner 构造一份 generic body，依赖 `LowerHir` schema 6 的模块指纹。当前 schema 为 5，body 携带 `large_copies`；`GirWorldV1` schema 3 在 fragment 之外保存 placement，指纹域为 `gugu-gir-world-v3`。`InstantiateGir` 仍从 HIR 收集调用边，不从 GIR 重解析。generic GIR 指纹（含 placement）进入 `ActionInputs` 与 `ImagePlan`；全程序分析仍消费放置前的 GIR 指纹。`LowerHir` 只构造、校验并冻结；单态化闭合、late、分析与 placement 在冻结之后按 `gir → mono → late → analysis → placement → summary` 顺序执行。
