...
`RuntimeRawModel`（query 30）当前为 schema 21，在同一 `RuntimeRawContractV1` 中并入
...
`RuntimeRawModel`（query 30）在该阶段升到 schema 12、当前为 schema 21，在同一 `RuntimeRawContractV1` 中并入
