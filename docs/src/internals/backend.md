...
Linux 镜像写出按 code RVA 顺序把每个 `UnwindFunction` 投影成一个 DWARF CFI FDE，并把 `LandingRecord` 写入该 FDE 引用的 LSDA call-site table；Windows 按相同顺序写一个 `RUNTIME_FUNCTION` 和对应 `.xdata`，landing 数据跟在 `UNWIND_INFO` 的 Gugu language-handler data 后。这两份平台表都从下面的统一字节投影，不能另排函数序。stack map `FunctionRecord.unwind_index` 就是该目标排序表中的 ordinal，必须与 function table 一一对应。Gugu panic unwinder据此选择 cleanup landing pad；外部工具/OS 使用平台表恢复寄存器。prologue code/offset必须满足 Windows UNWIND_INFO 限制。

### 统一元数据字节 {#unified-metadata-bytes}

代码生成在 branch relaxation 与指令编码之后，按分配位置填写每个片段的 safepoint、landing 与 source record，再按 mangled 符号升序把**进入栈图表**的函数拼成逻辑代码节。拼接从 RVA 0 开始，每个函数占 `[rva, rva + code_size)`，下一段起点向上对齐到 16 字节。`unwind_index` 等于该排序表中的序号。真正零 frame、无 safepoint、无 landing 的叶不进入这张表。镜像写出只能给整节加 load bias，不能改相对顺序。

根只来自该 safepoint use-slot 上仍有物理位置的活跃指针。`PollResume` 可以保留 GPR 根。`CallReturn` 只登记跨调用仍活跃的 `rbp`/`r12`/`r13` 根；调用者保存寄存器里的跨调用指针是后端错误，仅在该调用点死亡的参数寄存器不记入 caller map（由被调方 `MorestackEntry` 追踪）。suspend 与 `ForeignBridge` 上跨点仍活跃的 GPR 指针是后端错误。可重物化的 `StackAddr` 没有物理位置，不记成栈内根；栈地址在复制后由 `rsp` 重算。`CallReturn` 额外按真实目标的 ABI 分类登记传出栈参数根。`MorestackEntry` 的 `slot_count` 为 0，只登记函数 ABI 整数参数寄存器，PC 是 prologue 里 `call morestack_or_poll` 的起点。`PollResume` 的 PC 是 `call safepoint_slow` 之后的第一条指令。其它调用安全点的 PC 是该站点最后一条 `call` 之后的字节，必须小于 `code_size` 且落在指令边界。`PollFreeLeaf`、`ForeignLeaf`、纯分配与屏障不产生记录。无 default 的 `select` 是 `SuspendResume`，有 default 的是 `CallReturn`。`NoSafepointRegion` 内出现安全点是后端错误。同一根种类的重复槽位或寄存器位按位或合并；跨种类重叠、`r14`/`r15` 位与把 `rsp` 当根都是错误。landing 只来自 LIR `Invoke` 的 unwind 边：范围是调用指令 `[start, end)`，`landing_pc` 是目标块第一条非空字节，`cleanup_chain` 是目标块编号；目标是空的 `ResumePanic` 块时 `cleanup_chain` 为 `u32::MAX`。`panic_landing` 在 landing 非空时置位。`has_stack_interior` 在任一安全点的 `StackInterior` 寄存器或槽位非零时置位。当前普通函数的 `runtime_bridge` 为假。

栈图节继续使用 [栈图](stack-maps.md) 的 `GUGUSM01` / version 2，Linux 节名 `.gugu.stackmap`，Windows 节名 `.gugustk`，map 按完整记录字节字典序去重。统一展开表魔数 `GUGUUN01`、版本 1、逻辑节 `.gugu.unwind`。header 32 字节：magic、version `u16`、保留 `u16`、function_count、landing_count、functions_offset（固定 32）、landings_offset、section_len。每条函数记录 32 字节：`code_rva` `u64`、`code_size`、`frame_size`、`saved_gpr_mask` `u16`、flags `u16`（bit 0 panic landing，bit 1 runtime bridge）、`landing_start`、`landing_count` `u16`、保留 `u16`、保留 `u32`。landing 16 字节，字段顺序与 `LandingRecord` 相同。源码表魔数 `GUGUSR01`、版本 1，记录布局与 [GC 元数据](gc-metadata.md) 的 `SourceRecord` 相同，字符串池按路径字节去重后排序，逻辑上属于 `.gugu.meta`。header 32 字节：magic、version `u16`、保留 `u16`、record_count、strings_offset、strings_len、section_len、保留 `u32`。

`METADATA_SCHEMA = 1` 进入 fragment key。三节字节的指纹域是 `gugu-x64-metadata-v1`，并进入世界指纹。装配时用 runtime frame walker 审计节字节，再按每条 safepoint 与 landing 回读 PC。strip 必须保留目标栈图节、`.gugu.unwind` 与 `.gugu.meta`；只删除 `.text`、`.debug_info` 或 `.comment` 不得丢掉这三节，把栈图节本身列入删除集则不是合法 strip。镜像计划与 `-Zdump-x64` 暴露 schema、节名、字节数、函数/安全点/map/landing/源码记录计数与指纹。
...
[平台 ABI](../spec/platform-abi.md#executable-image-forms)给出 Linux external image profile。当前 writer profile 是 `ELF_SCHEMA = 1`，指纹域 `gugu-elf64-pie-v1`，不改变 `CODEGEN_SCHEMA`。它写出 `ET_DYN` static PIE：虚址等于文件偏移，页 4096，程序头放在虚址 64。首个 `PT_LOAD` 从 0 覆盖到只读段，因此 load bias 是 `AT_PHDR - 64`。首个 `PT_LOAD.p_vaddr` 为 0，不能拿它代替程序头虚址，否则会把这 64 字节算进 bias。rt0 把 64 编成立即数。

`.text` 从 `0x1000` 起。纳入栈图的函数按 mangled 符号升序、16 字节 NOP 对齐排在节首，`code_rva` 就是相对 `.text` 的偏移。其后是其余用户函数、按源名排序的运行时入口、`__gugu_trap`（`ud2`）、归档成员、恰好 6 字节的 PLT，最后是 `_start`。只读段含可选 `.interp`、哨兵字节、相对重定位表、常量与 `.gugu.stackmap` / `.gugu.unwind` / `.gugu.meta`。表的布局是 `u64` 计数加 `{offset, addend}` 对。rt0 只在 RELRO 虚址范围内写 `*(bias + offset) = bias + addend`，核对哨兵字节为 1，再把 `.data.rel.ro` `mprotect` 成只读。重复执行同一槽是错误。绝对重定位不能落在不可写节。

没有未解析的 C 符号时不写 `PT_INTERP`，即使描述符登记了解释器路径。有动态导入且描述符给出解释器时才写 `PT_INTERP`（`p_align = 1`）和 `jmp qword ptr [rip]` PLT。`PT_GNU_STACK` 不可执行。未知 `__gugu_runtime_*` / `__gugu_glue_*` 链接失败。static archive 按未解析 C 符号精确抽取；`staticlib` 写确定性 SysV archive，timestamp/uid/gid 为 0、mode 固定，成员按符号名排序。`cdylib` 仍按 ET_DYN 只导出显式 C 符号。

被引用的类型描述符、类型号、vtable、类型节基址和 global 在这条写出里是 8 字节对齐的只读占位，调用方可以把它的地址传进运行时。片段里的 `Symbol::Data(index)` 仍按函数内序号编码；写出时改按字节内容寻址，域是 `gugu-const-bytes-v1`。相同字节合并，对齐取较大值。序号只在所属函数的常量池里有效。

`_start` 走辅助向量、自重定位、RELRO，然后映射 8MiB `PROT_NONE` 栈并只把顶端 4KiB 改成可写。栈检查初值落在映射顶端，所以第一条带检查的序言会调用 `morestack_or_poll`，在保留映射里向下 `mprotect` 64KiB，而不是换掉已有帧的栈。协程块记着栈检查槽和映射下界；处理器块带上 TLAB 与 turn region 的 cursor/limit。然后才调用用户入口，正常返回用 `exit_group(0)`。`morestack_or_poll` 保留内部 ABI 的参数寄存器。`panic` 按 `{ptr, len}` 写到标准输出后 `exit_group(101)`。`channel_new` / `channel_send` / `channel_receive` 维护容量槽环。`gc_alloc_slow` 与 `gc_region_slow` 返回新映射的载荷指针。`spawn` 映射一块协程记录，写入代码指针与环境后返回。这条镜像只有一条可运行协程，所以 `yield`、`sched_park` 与 `sched_ready` 清掉 poll 字后回到调用方；`select_commit` 在没有等待队列时用 `rax = 0`、`rbx = 0` 报告第 0 个分支就绪。`coroutine_switch` 就是 `ContextSwitchCode::fixed()` 的 62 字节：`rdi` 保存、`rsi` 恢复、`rdx` 是 `CoroutineHot*`、`rcx` 是 `LogicalProcessor*`。这些入口都做对应的系统调用或内存更新。

通道对象在页尾 `+4088` 放关闭字。`channel_send` 在已关闭时 `exit_group(101)`；`channel_close` 重复关闭同样以 101 退出。`channel_try_send` 不阻塞：成功返回 `rax = 0`（`Ok`），满返回 `1`（`Err(Full)`），已关闭返回 `0x101`（`Err(Closed)`）。`channel_try_recv` 成功时 `rax = 0` 且 `rbx` 为取出的机器字；空为 `rax = 1, rbx = 0`，关闭后取尽为 `rax = 1, rbx = 1`。阻塞的 `channel_receive` 在空队列上仍 `exit_group(127)`，因为这条镜像没有把接收挂起。

值拷贝按描述符首个机器字给出的字节数执行。占位描述符该字为 0，因此拷贝长度为 0。`cow_snapshot` 在长度非 0 时映射新缓冲并复制，长度为 0 时返回原指针。`concat` 把两组 `(data, len)` 拼进新映射；两边都是 0 时返回空指针和长度 0。`format` 的计划仍是编译器序列化的文本，没有二进制计数字前缀时返回 `(0, 0)`。`wide_div` 做无符号 128 位除法，商的低位在 `rax`、高位在 `rbx`；除数为 0 时 `exit_group(127)`。有符号与余数变体目前与商共用同一符号。`utf8_boundary` 把 `rcx` 钳到 `rbx` 长度内，并退回到非续字节。`type_is` 比较对象首字与 `rbx` 中的类型描述符。`downcast` 命中时 `rax = 0`、`rbx` 指向 `+8` 的载荷，未命中时 `rax = 1`、`rbx = 0`；`downcast_copy` 把该载荷的第一个机器字放进 `rbx`。

`spawn` 的记录在 `+16` 记下是否已经运行，`+24` 保存返回值。`join_wait` 第一次等待时以环境指针调用代码指针，正常结束返回 `rax = 0`、`rbx` 为返回值；代码指针为空则 `exit_group(127)`。`defer_push` 映射 32 字节节点，动作在 `+0`、前驱在 `+8`、环境在 `+16`。平台 `reserve_aligned` 以 `(字节数, 对齐)` 映射页头和 `PROT_NONE` 载荷，返回页头地址；`commit` / `decommit` / `release` / `protect_guard` / `zero` 都从页头读出载荷地址和长度。`entropy` 用 `getrandom` 填满调用方请求的缓冲。`wait` 在字仍等于期望值时返回 0，不相等时返回 1，不把自己挂起。`wake` 返回 0。资源获取把处理器块上屏障槽之后 16 字节的计数加一，释放在计数为 0 时 `exit_group(127)`。发布、丢弃、pin 和 region 批次入口是 `mfence; ret`。镜像计划与 CLI JSON 暴露 `elf-schema`、`elf-type`、`elf-entry`、`elf-byte-count`、`elf-load-segments`、`elf-relative-relocs`、`elf-interp` 与 `elf-fingerprint`；指纹进入 action key。真实启动放在 `cargo bench --bench elf_pie`，默认测试套件只检查镜像结构。

### Windows PE32+

PE writer 消费[平台 ABI](../spec/platform-abi.md#executable-image-forms)给出的 PE32+、ASLR/NX、入口、逻辑节和导入导出要求。当前私有 writer profile 是 `PE_SCHEMA = 1`，指纹域 `gugu-pe32-plus-v1`，不改变 `CODEGEN_SCHEMA`。image base 为 `0x0000000140000000`，section alignment 4096，file alignment 512，COFF timestamp 与 checksum 为 0，subsystem 为 `WINDOWS_CUI`。`DllCharacteristics` 固定为 high-entropy ASLR、dynamic base 与 NX（`0x0160`）。这些字段不扩大平台稳定面。

`.text` 从 RVA `0x1000` 起。纳入栈图的函数仍按 mangled 符号升序、16 字节对齐排在节首，因此 `code_rva` 继续等于相对 `.text` 的偏移。其后是其余用户函数、按源名排序的运行时入口、`__gugu_trap` 与 `_start`。常量仍按 `gugu-const-bytes-v1` 合并，类型描述符等占位与 Linux 相同。逻辑节映射为 `.text`、`.rdata`、`.gugustk`、`.ggmeta`、`.xdata`、`.pdata`、`.reloc` 与一页 `.bss`。`.gugu.unwind` 的统一展开字节放在 `.xdata` 前部，后面是每个函数的 `UNWIND_INFO`。节名不超过 8 字节。`.text` 不可写。没有初始化的 `.data` 时不另开该节。

导入只来自已登记的 `kernel32.dll` 与 `ntdll.dll`，按 DLL 名再按符号名排序：`GetStdHandle`、`VirtualAlloc`、`VirtualFree`、`VirtualProtect`、`WriteFile`、`RtlExitUserProcess`、`RtlRandomEx`。调用是 `ff 15` 指向 IAT 的 RIP 相对位移，不写 syscall 号，也不扫描宿主导出。未登记的 C 符号是 `未登记导入`。未知 `__gugu_runtime_*` / `__gugu_glue_*` 一次报出全部名字。没有绝对地址时仍写出一个只含 `IMAGE_REL_BASED_ABSOLUTE` 填充项的合法 base-relocation 块；用户代码里的 `Abs64` 写成 `IMAGE_REL_BASED_DIR64`，槽内值是 image base 加目标 RVA 再加 addend。`Rva32` 与 `PcRel32` 不进这张表。当前主路径没有显式 C 导出，export directory 为空。`cdylib` 只额外设置 `IMAGE_FILE_DLL`。`staticlib` 是确定性 COFF archive（`!<arch>\n`，时间戳为 0），成员是同一份 `.text` 的 COFF 对象，不生成 `.lib` 导入库。

`_start` 先把入口栈对齐到普通被调用约定，再用 `VirtualAlloc` 建立 8MiB `PAGE_NOACCESS` 栈、协程块、处理器块、TLAB 与 turn region，并把栈检查初值放在映射顶端，因此第一条带检查的序言调用 `morestack_or_poll`，由 `VirtualProtect` 向下提交 64KiB。然后才调用用户入口。正常返回与失败都经 `RtlExitUserProcess`，后面跟着 `ud2`。这条路径不注册 console handler，`UNWIND_INFO` 的 flags 为 0，因此 SEH 回调不会进入 Gugu 用户代码。导入调用使用 32 字节 shadow，并在 `call` 前把栈对齐到 16 字节。用户函数若元数据里的 frame 非 0 且能找到第一条 `sub rsp`，`.pdata` 用 `UWOP_ALLOC_SMALL` 或 `UWOP_ALLOC_LARGE` 描述这次分配，prolog 长度取该指令的结束偏移；运行时入口自身的临时 `sub rsp` 使用 0 条 unwind code，避免把导入调用周围的保存误记成函数帧。

运行时返回约定与 Linux 写出相同：通道关闭字在 `+4088`，`channel_try_send` / `channel_try_recv` 的 `Result` 编码、`join_wait`、`defer_*`、值拷贝、`concat`、`utf8_boundary`、`wide_div` 与 `coroutine_switch` 都不改内部 ABI。分配、保护、释放和退出走 IAT。`platform_entropy` 在登记表里没有 `getrandom` 对应物，因此用 `ntdll!RtlRandomEx` 逐字节填充新映射的缓冲；这不是 CSPRNG，后续阶段若登记更强的熵源应替换这一入口。`platform_commit` / `decommit` 用 `VirtualProtect` 切换读写与不可访问，与 Linux 这条写出的 `mprotect` 剖面一致。镜像计划与 CLI JSON 在 ELF 字段之后暴露 `pe-schema`、`pe-entry`、`pe-byte-count`、`pe-sections`、`pe-imports`、`pe-relocs`、`pe-fingerprint` 与 `pe-staticlib-bytes`。PE 指纹紧跟 ELF 指纹进入 action key。默认测试只检查镜像结构；本环境不启动 PE。
