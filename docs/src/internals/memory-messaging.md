...
上述契约由 `ProvenanceRuntimeContract`（`RuntimeRawModel` schema 22 并入）与 `ProvenancePlane` 参照实现落地：检查目录、per-domain secret 管理规则、release/debug/security 三个安全 profile 的检查绑定、拒绝分类与统计口径见「Compiler 侧契约模型与 verifier」中的 provenance 契约段。`ProvenancePlane` 在 `RawWorld::new` 中以 release profile 建立——per-domain secret 由 world secret 经派生键 `gugu-provenance-domain-secret-v1` 逐 domain 推导、全零 secret 按初始化失败拒绝，每个 domain 持有独立的 `LinkCodec`，因此 encoded link 是 domain 绑定的；释放拒绝按 `chain-corruption`/`forged-link`/`double-return`/`cross-owner`/`cross-class`/`stale-generation`/`guard-region` 七类稳定记账，检查失败保持既有不变量路径、不静默丢弃消息。debug profile 在返还完成后写入 poison 标记（free 期间被改写的 slot 在分配时按链损坏拒绝）、用标记识别对已返还 slot 的重复返还、并对每次全链走查计数；security profile 用独立 runtime seed 在前 8 个 free slot 中随机挑选复用对象、提供 ForeignBridge 接缝的 checked copy 与 guard region 重叠检查；随机复用、guard region 与 checked copy 都不改变 managed trace 语义，只改变 raw slot 的复用顺序与额外记账。
...
- **压缩契约段**：`CompressionRuntimeContract`（`COMPRESSION_SCHEMA = 2`，profile `mosaic-compression` revision 2）固定 cage 位布局（cage id 8 / generation 24 / offset 32，全零字为空引用，generation 从 1 起）、cage 上限 4 GiB、与 GC arena 同源的 2 MiB 粒度、FFI 交接规则目录（`resolve-then-pin`、`no-compressed-pass-through`、`save-requires-active-lease`）与六项统计名，并逐项核对目标能力（`supported`、`canonical_bits = 48`、`min_alignment`）；契约同时固定 **cage 控制记录** 的字段表（`generation`/`cage_id`/`base`/`len`/`canonical_headroom`/`decodes`/`rejections` 的名字、字节偏移与宽度）与 `CAGE_CANONICAL_LIMIT`：机器解码序列按这张表读取记录，成功路径 `lock inc` `decodes`、拒绝路径 `lock inc` `rejections` 后走冷边，任何字段新增、重排或宽度变化都必须递增 `COMPRESSION_PROFILE_REVISION`，并由 `DecodeCompressedRef` 夹具与 `CompressionPlane` 的 checked 解码逐个对照。`CompressionPolicyV1` 默认关闭，关闭态要求 cage 字节数、解码点与压缩根槽同时为 0。`CompressionDemand` 由优化后 LIR 一次推导：`decode_sites` 是 `DecodeCompressedRef` 指令数，`compressed_root_slots` 是按安全点聚合的压缩根槽数——两者随需求进入 `RuntimeRawModel` key、契约指纹与 `ImagePlan`，`cage_bytes`、能力或需求变化必须使缓存与 action key 失效。默认编译不预留 cage。
