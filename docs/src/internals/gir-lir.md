...
`AbstractAnalysis` 在 generic body 上求固定点，证明写入 `AnalysisWorldV1.proofs`。单态化替换仍由 `InstantiateGir` 从 HIR 收集边；不得从 HIR 再造一份平行分析 CFG。
...
`std.text.Bytes` 与 `std.io.ByteBuffer` 走上面的 `CowSnapshot`。复制封存 backing，之后修改的一侧分离，旧快照字节不变。runtime 参照模型按 Unicode 17.0.0 判定 UTF-8/UTF-16、lossy U+FFFD、负索引、标量边界、默认大小写、完整 case fold、NFC/NFD/NFKC/NFKD，以及 grapheme 与 word 边界。行切分目前只实现强制换行（CR LF 保持在一起，并在 LF、NEL、LS、PS 和不跟随 LF 的 CR 之后断开），不是完整的 UAX #14 类对规则。这些函数还没有降成可从 Gugu 源码调用的 intrinsic。

`std.collections` 的 `HashMap` / `HashSet` / `SecureHashMap` / `SecureHashSet` / `BTreeMap` / `BTreeSet` / `SmallMap` 是按名字登记的共享身份 lang item：passing class 为 `IDENTITY`，复制只复制句柄，不产生 `CowSnapshot`。接收者类型为这些集合的 `with_ref` / `for_each_ref` 派发在调用点降为 `ScopedViewBegin ScopedRead`（源是接收者 place）、`Call`、`ScopedViewEnd`，整个调用位于 view 的动态 extent 内；调用的 unwind 边先进入一个只做 `ScopedViewEnd` 的 cleanup landing 块再接外围 cleanup 链，因此正常返回与 panic 展开两条路径都闭合 token，满足 GIR verifier 的配对 / no-suspend 规则与 LIR verifier 的出口无未闭合 guard 规则；`std.mem.with_ref` 等旧钩子继续按名字产生 view。集合的键约束（`Eq + Hash + StableHash`、`Ord + StableOrd`）由内建源单元 `std/collections.gg` 的 impl 头声明并走普通 trait 求解，缺少 marker 的键类型没有适用方法。内建源单元可以声明预导入名，用户源码不能。runtime 参照模型 `runtime/collections.rs` 固定语义：共享句柄、普通读取返回语义副本、`with_ref` / `for_each_ref` 期间任一别名的写入是 panic、`iter()` O(1) 封存 backing 且第一次修改先分离、Entry 只传递副本；HashMap 类按所选哈希族的键 hash 排列实现序，BTreeMap 类按键全序，SmallMap 在内联阶段按插入序线性查找、溢出后与同族 HashMap 相同。`runtime/hash.rs` 固定 `Hasher` 的语义馈送（定宽标量、带长度前缀的文本/字节），以及默认 FoldHash-fast（进程熵 seed）、SecureHashMap 的 SipHash 1-3（CSPRNG key）与稳定的 XXH3-64 / XXH3-128 三类哈希族；前两者不是持久格式。`runtime/fmt.rs` 固定 `Formatter` 的 padding、integral 与结构化 debug 写入。这三组参照模型还没有降成可从 Gugu 源码调用的实现；`std/collections.gg` 里的方法体是类型检查用的桩，`SmallMap[K, V, comptime N]` 的 comptime 值参数也尚未进入类型形成。

管线为 `BuildGenericGir` → `mono` → `late` → `attach_fragments` → `WholeProgramAnalysis`（schema 5，`analysis_semantics_revision = 5`）→ `EscapeAndPlacement`（query **29**，schema 2，域 `gugu-escape-placement-v1`）→ `PublicFunctionSummary`。placement 只记录、不改写 CFG。分析使用放置前的 GIR 指纹，避免循环。`GirWorldV1` schema **3** 携带 `PlacementWorldV1`；world 指纹域为 `gugu-gir-world-v3`。未逃逸槽为 Stack；`address_taken` 且引用导出时按 publish / foreign / 其它分别选 SharedHeap / Pinned / LocalHeap。含 resource passing 的对象与数组分配，以及逃逸的 Resource placement，必须进入独立 Resource domain，不得改写成 LocalHeap 或 managed region；unknown / alias / foreign / escape / publish 按相应 managed 规则选择。分配点无私有证明不得选 `TurnRegion`；`TurnRegion` 要求 `Proved` 且无 UNKNOWN|PUBLISH|FOREIGN|RESOURCE|ALIAS。纯位 `ValueAction` 不进入分析 heap/alias，以免破坏范围证明。

placement schema 2 在分配点之外新增 region 计划：CFG 去掉直接挂起（`Suspend`、`SelectCommit { suspend }`）与出口（`Panic`/`Return`/`ResumePanic`/`Abort`/`Unreachable`）后按连通段分段，每段至多一个 region 计划，记录该段的分配点、声明 export summary、出口列表与哪些出口整体移交所有权（`RegionExit.transfer`）。可调用边界不是 region 出口：调用的 callee 只在调用者下一次直接挂起或出口前返回，期间调用者不会 reset。分配点必须支配段的每个出口；出口处终结符读取的、能追溯到分配点的局部必须全部是 send 值，且任何可达局部都不得在出口之后仍 live，否则该段不得选 `TurnRegion`。被 channel send 读取的值带 `CHANNEL_SEND` 位而不是 `PUBLISH|ESCAPE`：只有「sender 之后不再使用」才留下 region 并标记移交，否则退回 `SharedHeap`。带环境的闭包才产生环境分配点；无捕获的函数项引用不分配。`ImagePlan` 增加 `placement-count`、`turn-region-count`、`local-heap-count`、`shared-heap-count`、`placement-fingerprint`，以及 TurnRegion 契约的 `turn-region-sites`、`turn-region-publish-sites`、`turn-region-reset-sites`、`turn-region-promote-sites`、`turn-region-transfer-sites`、`turn-region-capacity-class-count`、`turn-region-object-limit`、`turn-region-max-bytes`、`turn-region-total-bytes`、`turn-region-contract-fingerprint`、`turn-region-demand`。`-Zdump-gir` 打印 ValueAction / ResourceAction / CowSnapshot 与 placement 表。堆装箱改写与 monomorphic GIR 替换尚未物化。
