...
官方工具链在形成镜像计划前还必须校验具体实例的低层表示：值的定义与使用、内存效果顺序、引用来源、清理出口和内部 effect region 都必须成立。冷编译与缓存恢复使用相同的校验入口；非法内部表示属于编译器内部错误，不能写出镜像计划或以另一条生成路径继续执行。该校验不引入用户级 move/borrow 门槛，也不能拒绝本来合法、需要延长存储寿命的引用。
...
可达边按具体实例的 body 归属收集：函数值即使经过擦除、作为参数传递或存入聚合，仍保留
其具体函数实例；闭包、协程 body 和局部 static 初始化器各自拥有调用边。一个接口的不同
具体接收者分别保留 vtable。泛型 body 的方法选择在类型实参闭合后重新确定，函数的
comptime 值实参也参与实例身份；不能把仍带类型占位的声明当作已闭合实例。
...
late 依赖从已经可达的具体实例继续闭合，不使未使用的泛型声明成为无实参根。
全程序摘要按实例调用图的 SCC 求解，调用点只消费实际选中实例的已完成摘要。
共享定义上的安全检查只有对全部可达实例都已证明时才可省略；未知调用、未收敛 SCC、
运算符与协议派发的副作用不得被遗漏。公共摘要必须保留参数与私有状态的读写效果，
但不得把私有定义或本次编译的稠密编号作为可消费事实。
...
同一 compiler session 内修改源码后，单态化与公共摘要求值必须与重新编译一致。
实例身份、签名/ABI 指纹和代码片段输入指纹分别表达实例、接口与实现；实现变化必须
使片段重新生成，而只改变无关定义编号不能改变公共摘要对象身份。
