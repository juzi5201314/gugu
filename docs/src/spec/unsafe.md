...
`unsafe fn` 的调用前置条件跟随函数项身份，赋给推断出的局部函数值后仍然有效。函数体中的原始指针访问等不安全操作也必须写显式 `unsafe` 块。普通 `fn(...) ...` 与 `Fn(...) ...` 表示可安全调用的签名，不能用 unsafe 函数项直接满足；需要安全接口时，应由安全包装函数在内部维护不变量并执行 unsafe 调用。动态接口方法保留各自声明的 unsafe 前置条件，不能通过 `dyn` 擦除消除它。
...
只产生裸地址的构造不要求 `unsafe`：`(*U)(p)` 可以改变裸指针的指向类型，`(*U)(r)` 可以取引用的地址，`(*T)(addr)` 接受 `uint` 地址，`uint(p)` 可以读取裸指针地址值。这些操作不读取指向的值，也不证明其有效性。
...
链接属性按声明种类检查：`link_section` 同时适用于有体函数、模块 `static` 和 `global_asm`，`used` 只适用于有体函数与 `static`。同一链接属性不能重复，符号名与节名必须非空且不能含 NUL。PE 镜像节名按编码后的字节数限制为 8；目标格式的导入、导出、重定位、展开、类型及 runtime metadata 保留节不能由 `link_section` 占用。
...
编译器检查显式取引用及方法接收者自动借用的完整位置投影：已有引用提供被指类型的对齐保证，字段偏移、固定数组下标及动态数组步长决定最终地址的可证明对齐。`packed` 配合 `align(N)` 只增加容器起点与数组步长的对齐，不填充内部字段；无法证明字段自然对齐时拒绝形成引用。此静态检查不重新求值下标，也不替代运行时边界检查。`addr_of` 不形成引用，不受该对齐限制。
