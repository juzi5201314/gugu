...
该禁止降级检查作用于模块、声明和表达式的词法嵌套范围，不会泄漏到兄弟声明。`cfg` 已删除的节点不参与 lint 级别的继承或降级检查；已通过词法、语法校验的 inactive 子树不会重新进入诊断级别分析。

lint 级别只由 `allow`、`warn`、`deny`、`forbid` 的参数列表决定。`#[used]` 等无参数属性和 `repr` 等其它属性不形成 lint 作用域，也不能被当作 lint 参数列表读取。
...
`f"..."` 的 `{` `}` 里是完整表达式；表达式后可以写一个冒号和 Rust 风格静态格式说明，例如 `f"{id:08x}"`、`f"{value:?}"`、`f"{ratio:.digits$e}"`。格式码在编译期选择 `Print` / `Debug` / 进制或指数格式 trait；width 与 precision 的 `name$` 必须引用当前作用域的 `int` 绑定。表达式里的括号、方括号、花括号按嵌套配对；未配对且不属于格式说明的 `}` 结束插值。禁止在插值表达式里再写 `f"..."`。完整格式规则见[标准库 · 静态格式化](standard-library.md#static-formatting)。

格式计数的 `name$` 是对绑定值的实际读取，参与与插值表达式相同的确定初始化和闭包/async 捕获分析；只在格式说明中使用的名字也不能绕过这些规则。固定 width/precision 必须处于非负 `int` 范围；动态负值仍按静态格式化规则在运行时 panic。
...
## 属性适用位置与冲突 {#attribute-placement-conflicts}
...
编译错误和升为错误的 lint 必须使编译器返回非零状态且不得留下可执行镜像。警告不改变返回值或程序语义。诊断至少包含级别、稳定的诊断名字、主消息、主源范围；涉及重名、impl 重叠、类型来源或属性覆盖时还必须标出相关的次级范围。行列号按 1 基计算，列号基于该行的 Unicode 标量计（非字节偏移），Span 端点必须落在 UTF-8 字符边界上。

同一输入的诊断顺序必须确定：先按规范化文件路径、主范围起始位置、错误优先于警告、诊断名字排序。并行解析、单态化或测试收集不能改变顺序。UTF-8 文本输出写到 stderr；实现可以另提供机器可读格式，但不能以机器格式替代规范文本中的物理源位置。文本布局（源码片段、插入符、缩进与颜色）是展示细节，不是稳定接口；它不能替代本节要求的级别、诊断名字、消息与主源范围。

解析恢复可以继续报告后续错误，但不得把一个已知语法错误产生的占位节点当作有效程序进入 codegen。实现可以限制单次报告数量，达到上限时必须明确说明仍有诊断被省略。

词法诊断使用稳定代码 `E0009`–`E0019`：未闭合字面量或属性括号、未闭合块注释、非法转义、非法数字、非法记号、未知属性、非法属性参数、非法 Unicode scalar、C 字符串内嵌 0、非法字节字符、非法 f-string 格式说明。它们与源码快照诊断 `E0001`–`E0008` 一起按本节顺序排序。按值传递超过 64 字节的位结构体使用 `E0056`（lint `large_copy`，默认 warn）。

源码快照与 bootstrap 结构诊断使用稳定代码 `E0001`–`E0008`：`E0001` 源文件无法读取、`E0002` 缺少可执行入口、`E0003` bootstrap 无法识别源码结构（例如 target 源码树中的符号链接）、`E0004` 非法 UTF-8、`E0005` 源文件含 BOM、`E0006` 非法逻辑路径、`E0007` span 越界、`E0008` 源文件过大。
