...
编译期函数和常量初始化采用定义处的词法作用域，不可读取调用者的局部同名绑定。
`return`、`break`、`continue` 与失败的 `?` 必须立即离开当前求值上下文；未求值的
兄弟操作数、后续实参和赋值不得执行，也不能覆盖正在传播的退出结果；`break expr`
...
固定片段类别的解析入口（`parse_items` 等）返回的片段用于其它 slot 时报告
`expansion-fragment-mismatch`（`E0050`）。语句片段的可选尾表达式只能成为所在块
末尾语句之后的块尾；宏不是所在块最后一条语句、或该块已有尾表达式时，同样是
`E0050` 展开错误，不是普通类型错误。
...
源码宏最终返回 `Err` 时，编译器不展开该宏，并把错误转换为编译诊断
（`macro-boundary-error`，`E0051`；`Err` 负载的文本取字符串或 `message` 字段）。
宏脚本 panic、调用禁止的编译期操作、fuel 耗尽或编译期内存耗尽属于宏执行错误；
解析成功后生成片段的名称解析、类型检查、初始化、trait、`unsafe` 和 ABI 错误属于
展开源码的普通编译错误。生成片段内的 `#[cfg]` 谓词在拼接当轮按同一 `cfg` 上下文
求值；被裁掉的生成项不进入名称解析。
宿主中被 `cfg` 删除的声明、参数、字段或分支内，所有 source slot 均不得展开；
不执行其脚本、不登记生成源码，也不扣除展开预算。
...
完全相同的宏脚本文本再次出现在当前展开栈上时，编译器报告确定性展开循环
（`expansion-cycle`，`E0048`）并附完整展开链；不同已知输入的递归可继续，直到任一
预算耗尽（`expansion-limit`，`E0049`）。属性值必须是 `1..=`全局硬上限内的正整数；
超出范围、非正或同一作用域重复指定不同值都是 `E0049` 编译错误，不得静默截断或
取整。当前实现的全局硬上限为 256，默认深度上限为 16。

生成片段中的内层 `comptime(expansion_limit = N)` 属性同样必须校验；生成子树继承
调用点有效上限，子树属性可以继续收紧，不能突破祖先限定的子树预算。
缓存命中与实际执行扣除相同的 fuel 和 heap；一次 action 的总预算耗尽不能污染
其它 action 对同一宏脚本的求值结果。宏输入中常量或被调函数实现的改变必须使缓存失效。
...
复合赋值必须按原值与右操作数推导新值；整数范围不得忽略实际位宽的回绕。
可能经引用改写标量的调用同样使旧标量事实失效，而非仅使对象长度失效。
函数返回范围必须合并所有可达返回出口；仅在相应参数没有改写或未知别名干预时，
才能把返回值与参数长度之间的关系传给调用点。
...
聚合槽、重复元素的递归负载、局部值深复制、字符串拼接与插值均计入确定性 heap
预算；求值器必须在重复分配或扩容前检查配额，不能仅按聚合元素数量计字节。
