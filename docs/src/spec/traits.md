...
关联调用遵循[类型路径的分段泛型规则](types.md)。`Self` 在关联项签名和函数体中都指当前实现的完整 Self 类型，可以直接用于构造该类型。
...
已受约束的 `T::Name` 只有在该约束集合能唯一确定 trait 实例时才形成投影。不同 trait 即使定义相同名字、并在某个具体实现中选用相同类型，也不能因此合并为一个候选。固有 impl 上的泛型约束与 trait impl 的约束一样，在成员选择时必须有证据；不能因为成员存在就跳过约束检查。
