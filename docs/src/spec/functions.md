...
省略返回类型的闭包若有期望 `fn(...) R` 签名，使用该签名约束返回值；没有期望签名时，表达式体 `fn(x) = expr` 从 `expr` 推断非 never 的返回类型，块体默认 `()`。无论体的形式，不能仅因体发散就把省略的返回类型推成 `!`；需要 never 函数类型时显式写出 `!`。
...
函数项的具体类型身份还包含该实例的全部泛型实参；即使某个实参未出现在参数或返回类型中，不同实参的函数项也不是同一种具体类型。值后面的 `[]` 仍是下标，具名函数的显式实例化必须写 `::[...]`。
...
初始化检查把闭包构造和闭包执行分开：构造时只建立对实际外层槽的捕获关系，不把体内的赋值算作外层当前路径已经赋值。直接调用一个已知闭包时，调用前必须满足其对捕获槽的初始化要求；递归闭包的自身槽在构造完成后才满足该要求。`async` 调用形式在父协程求值调用目标和参数，块形式在子协程执行 body；子协程中的赋值不能证明父协程随后读取的槽已初始化。

把 callable 传给可能调用它的函数，或从当前函数返回 callable 时，同样需要满足捕获槽的初始化前置条件。返回边界在普通 defer 完成后检查这些条件，因此 defer 完成的赋值对随后使用返回闭包的调用方有效。新协程可能立即执行，不能用父协程在启动之后才进行的赋值证明子协程捕获已初始化。
...
类型参数包必须位于泛型参数列表末尾，变参包必须位于函数参数列表末尾。显式实例化时，固定类型参数后的剩余实参形成类型参数包；调用点按包中每个位置的具体类型检查实参，不执行统一装箱。
