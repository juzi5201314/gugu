...
见[值、句柄与传递](passing.md)。`f(x)` 按值描述符产生语义副本，`f(&x)` 引用槽。普通函数不自动取地址；方法接收者是 `&Self` 时 `p.m()` 可以插 `&p`。关联函数写 `Type::m()`，见[接口](traits.md)。
...
调用时，实参按[表达式与语句](expressions.md)从左到右求值，再按值描述符复制或按引用写进参数槽；不可驳参数模式随后在 callee 入口建立绑定。参数模式理论上失败即程序不是良构程序，因此没有运行时“参数匹配失败”分支。返回表达式按声明的返回类型检查并在 defer 展开前物化；resource drop glue 在返回值完成 transfer 或复制后才处理退出槽。
