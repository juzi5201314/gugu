...
每段路径的泛型实参只属于该段：`Box[int]::new()` 中 `int` 绑定 `Box`，不会移到 `new` 上；`Box[int]::method::[string](...)` 中第二组实参才绑定方法自己的泛型参数。关联类型投影同样保留前缀 trait 或类型的实参。
...
`isize` / `usize` 在已支持目标（本版本登记的两个目标，见[平台与 ABI 参考](platform-abi.md)）上与 `int` / `uint` 同宽，因此并进上表。`i8`、`i16`、`i32`、`i128`、`u16`、`u32`、`u128`、`f32` 各是不同类型。`i128` / `u128` 在 x86_64 上对齐 16。
...
`i128` / `u128` 可以出现在 Gugu 函数、结构体、数组里。它们**不是**日常类型。`extern "C"` 的签名里：`x86_64-linux` 按 SysV `__int128`（`rdx:rax`）；`x86_64-windows` 的 C ABI 没有 `__int128`，把 `i128`/`u128` 写进 `extern "C"` 是编译错误。要过 Windows C 边界，拆成两个 `u64` 或 `#[repr(C)]` 结构体。完整映射见[平台与 ABI 参考](platform-abi.md)，不要抄 Rust 自己的 `i128` 传参。
...
无 `#[repr]` 时编译器可重排字段与压缩枚举，但在「同一目标 + 同一编译器版本」上确定。跨语言用 `#[repr(C)]`；C 布局和外部调用约定见[平台与 ABI 参考](platform-abi.md)。
