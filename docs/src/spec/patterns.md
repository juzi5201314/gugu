...
模式出现在 `match`、`if let`、`while let`、`let` / `let-else`、函数与闭包参数里。匹配成功时字段与元素按[传递](passing.md)的值描述符复制到绑定（与读 `p.x` 相同），不是自动变成 `&T`。没有 `ref` / `ref mut`。
...
| `_` | 丢弃。同一作用域可出现多次。 |
| `x` | 绑定。默认可变，引入新槽。 |
| `x @ 子模式` | 先按子模式匹配，成功则把整个被匹配值按值描述符复制到 `x`。 |
...
- `match` 与 `if let` 会自动解一层或多层 `&`：`opt: &Option[T]` 时 `match opt { Some(x) => ... }` 合法。绑定仍按[传递](passing.md)的值描述符产生 T 的语义副本。模式里的 `&P` 仍可显式写。
...
模式成功后，绑定按从左到右、从外到内的结构顺序建立为新槽；每个绑定对匹配值执行一次值描述符复制。`x @ P` 先验证 P，成功后再复制整个值给 x。模式失败时不建立任何绑定，也不保留部分 COW seal 或 resource lease；编译器必须把管理动作推迟到模式确认成功之后。
