...
| `_` | 丢弃。同一作用域可出现多次。 |
| `x` | 绑定。默认可变，引入新槽。 |
| `x @ 子模式` | 先按子模式匹配，成功则把整个被匹配值的语义副本绑定到 `x`。 |
...
| `&P` | 匹配 `&T`：按 `P` 匹配所指的 T，并按值传递规则建立绑定。 |
...
- `match` 与 `if let` 会自动解一层或多层 `&`：`opt: &Option[T]` 时 `match opt { Some(x) => ... }` 合法。绑定仍按[传递](passing.md)规则产生 T 的语义副本。模式里的 `&P` 仍可显式写。

自动解引用由结构模式触发；简单绑定 `x` 和 `x @ P` 的整体值保留当前位置的类型（包括 `&`），不会把引用绑定悄悄改为被引用值。解构到字段或元素后，子绑定取得对应字段或元素的类型。
...
穷尽性按静态类型计算：`bool`、枚举和有限数组形状精确枚举；整数/字符范围按区间并集计算；带运行时守卫的臂不贡献覆盖。无限标量域无法由有限离散字面量穷尽时必须有覆盖剩余值的范围或 `_`。空范围、端点类型不同、下界不小于上界、重复绑定名、or 两侧绑定集合或类型不同都是编译错误。重叠臂合法，仍由第一匹配臂获胜。

结构体模式省略字段时必须写 `..`；不写 `..` 就必须列出所有当前可见字段。私有字段在定义模块外不能被点名，但可以由 `..` 忽略。数组精确模式必须与长度一致；切片 rest 模式只产生原切片的视图，不分配或深拷载荷。

## 覆盖分析的边界

覆盖分析使用模式自身的内置构造关系，不调用用户定义的 `Eq`、`Ord` 或其它方法。整数按类型完整有限域与半开区间分析，`char` 域排除代理项；空 enum 和 `!` 无可构造值。元组、enum、数组和切片按各分量组合分析，不能把分量分别覆盖误认为整个乘积已覆盖。

切片的前缀与后缀约束可能在短切片上重合，也可能在较长切片上分离，分析必须涵盖两种情况；任意大固定数组的无关元素不要求逐个枚举。守卫不提供结构覆盖证明，即使守卫写成 `true`，也仍需要无守卫的完整覆盖。
