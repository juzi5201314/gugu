...
`GUGU_RUNTIME_MEMORY_LIMIT` 是软上限，不是操作系统硬隔离。它覆盖 managed heap、runtime私有 metadata、用户协程栈等 runtime管理的已提交内存，不覆盖镜像代码、操作系统内核资源或外部库自行分配的内存。runtime可以因当前对象、页粒度和并发回收周期暂时超过该值；超过 `pressure_enter_ratio` 后进入新的 pressure episode，并按 owner drain、collection、trim/decommit 的有界预算推进；episode 内的 owner drain 按 `owner_drain_items`/`owner_drain_bytes` 的单 shard 预算和 `owner_drain_interval_bytes` 的分配间隔执行，committed 快照按 `pressure_poll_bytes` 的间隔刷新。占用降到 `pressure_clear_ratio` 以下且 `pending_return_bytes`、`owner_cache_bytes`、`reclaimable_bytes` 三类都各自完成一次真实 owner drain 后才结束该 episode；未配置软上限时 `pressure_debt` 固定为 0，但 pending return drain 与平台分配失败仍按本节规则处理。
...
分配请求触发 soft limit 时，当前 pressure episode 至多启动一次 forced full cycle 决策，且只有在该 episode 真正完成一次 owner drain 之后才允许结束 episode、由下一个 episode 再取得一次 forced cycle 机会；请求的字节数与当前 committed 之和必须先超过 soft limit，仅请求本身越过 limit 时进入 drain 而不是 emergency。forced cycle 完成后若仍高于 clear 水位，runtime继续执行有界 owner drain、forwarding grace、sweep、trim/decommit和有限 assist，不在每次临界分配上重复重扫整堆。仍不能取得 headroom时进入 `OutOfMemory`；占用低于 `pressure_clear_ratio` 且 pending/cache/reclaimable 已完成 owner drain 后，才允许下一 episode 再启动 forced cycle。显式 `std.runtime.collect()` 也不保证所有内存返还宿主，不改变任何存活值或安全引用的语义，也不运行用户 finalizer。cycle 只有在全部 owner 的 root snapshot 收齐、且本地 worklist、已发布 batch、mailbox、barrier buffer、producer epoch、在途转发与未归还 credit 七项收敛条件全为 0 后才允许完成；跨 owner 的可达对象由只携带稳定身份的工作消息交给持有者标记，因此一条 cycle 不会遗漏别的 owner 的对象。mark 未收敛时本轮 cycle 不推进 credit epoch，但这不改变分配成功与否，也不使 `collect()` 失败；堆内存的回收可在后续 cycle 追平。具体 safepoint、heap和 cycle阶段见[GC内部规范](../internals/gc-metadata.md)。
allocation debt 只影响自动 collection 的节奏；memory limit 触发的 drain、emergency sweep、decommit 和 `OutOfMemory` 规则不受 `GcTarget::Off` 关闭。`GcPacingProfile` 的 `min_growth_budget`、`assist_threshold`、`assist_quantum`、`mark_cost_per_byte`、`gc_cpu_fraction`、`gc_cpu_window_cost`、`remark_cost_budget`、`evacuation_pause_bytes`、`evacuation_pause_roots`、`evacuation_pause_fields`、`pressure_enter_ratio`、`pressure_clear_ratio`、`pressure_poll_bytes`、`owner_drain_items`、`owner_drain_bytes` 和 `owner_drain_interval_bytes` 是编译期登记的实现门禁，与 `CompilerIdentity` 和 runtime tuning profile 一起版本化；它们是内部预算，不是用户可观察的时间单位。具体 debt 公式和 pressure hysteresis 见[内存所有权与消息通道](../internals/memory-messaging.md#allocation-debt-pressure-backpressure)。
...
struct RuntimeStats {
    ...
    pub compression_decode_rejections: uint,
    pub compression_foreign_pins: uint,
    pub compression_foreign_saves: uint,
    pub compression_foreign_copies: uint,
    pub compression_foreign_rejections: uint,
...
`turn_region_resets` 只统计通过 export summary 验证的整区 reset；`region_promotions` 统计经 `LocalPromote` 保留的 region。一个私有 region 只有在状态为 `Publishing`、transfer lease 为 0，且 export summary 的五位（`external-alias`、`resource-lease`、`ffi-address`、`pending-transfer`、`live-root`）全为 0 时才能 reset；任一位非零或仍有在途 transfer 时必须保留该 region 而不是释放它，因此 `turn_region_resets` 与 `region_promotions` 的划分由这组可观察条件决定，而不是由调用点选择。region 移交只在真实的 channel send/select 边界发布 `RegionTransfer` 消息；接收 owner 采纳前 region bytes 计入 `pending_return_bytes`，采纳后两侧 owner 的 committed bytes 一起移动，任何一侧都不得在此期间把它当作可复用空间。

`runtime_committed_bytes` 不包含 managed heap；limit 判断读取 runtime 管理内存的 provider committed 总量（raw extent、协程栈 arena 与 managed heap 由同一 provider 提交，已经去重），`classed_committed_bytes` 与 `stack_committed_bytes` 是它的两个互斥子集，只用于覆盖校验而不是相加。`pending_return_bytes`、`owner_cache_bytes`、`reclaimable_bytes` 和 `range_reserved_bytes` 与既有 heap/stack 统计分开报告。`remote_return_batches`、`remote_return_hops`、`forwarded_messages`、`mark_ticket_batches`、`edge_delta_batches`、`mark_credit_pending`、`shared_handle_resolves`、`shared_handle_forwards`、`turn_region_resets`、`region_promotions` 和 `compressed_ref_decodes` 分别记录 owner return、GC message、handle、region 和 cage profile 的诊断口径；`compressed_ref_decodes` 只统计 checked 解码路径上的成功次数，空压缩字不解码也不计数，被拒绝的解码计入 `compression_decode_rejections`。cage profile 的 FFI 交接另有四项计数：`compression_foreign_pins` 统计建立的 pin lease、`compression_foreign_saves` 统计通过活动 lease 完成的保存、`compression_foreign_copies` 统计成功的受检复制、`compression_foreign_rejections` 统计被拒绝的保存与压缩字直接交接；复制要求地址落在已知 cage 内，但它不延长地址生命周期，因此不建立 lease。`mark_credit_pending` 是本 cycle 尚未归还的 owner credit 总数，它不对应可从 committed bytes 中扣除的内存，也不构成同步、调度或回收时刻承诺。managed heap 的 32 KiB block 与 raw extent、协程栈 arena 共用同一个 provider 提交，因此已计入 provider committed 总量而不重复进入 `runtime_committed_bytes` 的账本分区；heap 自身的 `heap_committed_bytes`/`heap_live_bytes` 统计与 4 KiB page 级地址解析由后续阶段补齐。

checked pointer compression 只有在显式 cage profile 下才存在，该 profile 是编译期契约的一部分：默认关闭时不存在 cage，压缩引用与完整地址语义等价，世界不预留任何 cage 地址，也因此不改变 `range_reserved_bytes`。开启时 runtime 预留一个不超过 4 GiB、按 GC arena 粒度（2 MiB）对齐的 cage，managed arena 以 island 形式从 cage 切出，压缩字把 cage 内引用编码为 `cage id`（8 位）、`generation`（24 位）与 `offset`（32 位）；全零字是空引用，generation 从 1 起，编码字段总和固定 64 位。cage reservation 只计入 `range_reserved_bytes`，island 内的 extent 提交后才进入 `runtime_committed_bytes`，两者仍严格互斥。解码是 checked 路径：未登记 cage、过期 generation、越界 offset 与落在 canonical hole 的地址都进入 `RuntimeInvariant`，绝不退化为按位域猜测；cage 跨越 48 位 canonical 边界时，边界以外的部分不可解码。压缩引用不能直接交给 native code，外部代码只能取得已 resolve 的完整地址，且把它保存到调用生命周期之外必须持有活动 pin lease；用租约保存与受检复制分别计入上述 FFI 统计。
