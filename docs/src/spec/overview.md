...
- **运行时 panic**：程序执行了语言定义的失败操作，例如越界、除零、无效浮点到整数转换、关闭 channel 后发送或重复关闭。panic 可以被 `Join.wait()` 或 `std.panic.catch` 收集；未收集时按[运行时与运维语义](runtime.md)处理。
- **运行时 fatal**：runtime 无法安全恢复的资源、栈、平台或内部不变量故障。fatal 不能被用户捕获，会停止整个进程，并按[运行时与运维语义](runtime.md)报告。
...
`cfg` 删除发生在名称解析之前；早期 comptime 选择发生在类型检查之内，不能让未选分支逃避类型检查；`comptime source` 先运行编译期脚本并解析生成片段，再回到 `cfg`、定义收集、名称解析和类型检查。单态化闭世界与具体类型集合冻结后才运行受限的 late comptime，且不得反向改变前述结果。所有程序可达的声明和标准库/runtime 声明必须在闭世界编译中可见，见[程序与编译模型](program-model.md)。
