...
`cfg` 删除发生在名称解析之前；普通 `comptime` 选择发生在类型检查之内，不能让未选分支逃避类型检查；`comptime source` 先运行编译期脚本并解析生成片段，再回到 `cfg`、定义收集、名称解析和类型检查。所有程序可达的声明和标准库/runtime 声明必须在闭世界编译中可见，见[程序与编译模型](program-model.md)。
