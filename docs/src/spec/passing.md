...
`f(big)` 对 1KB 的结构体仍然是浅拷贝，合法。编译器必须对超过实现阈值（建议 64 字节）的按值传递发出 lint `large_copy`，提示改 `&T`。这不是类型错误。可用 `#[allow(large_copy)]` 关掉某一处。热路径靠诊断 + 逃逸分析，不靠 Clone 门槛。诊断通道见 [词法 · 诊断](lexical.md)。
