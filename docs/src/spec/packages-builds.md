...
实现中的 `ActionInputs` 按规范固定编码全部影响输出的输入，集合/映射按编码键排序，使用 `gugu-action-v1` BLAKE3 派生 `ActionKey`；缓存命中只改变输入来源，不改变 key 或依赖输入字节。`TargetView` 物化到 workspace target 视图，并拒绝绝对路径与目录穿越。
