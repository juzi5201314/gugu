...
3. **风格 lint**：不改变类型或运行时语义的命名、文档和可读性诊断。默认级别见[风格 lint](#style-lint)。

规范排版不得改变程序的语法结构（连续 `use` 区域的规范排序除外）、字符串和字节常量内容、文档注释正文或属性参数值。格式化后 `std.src.file()`、`line()` 和 `column()` 仍按实际格式化源文件报告调用位置；排版改变物理行列是允许的，不得改变这些 API 的计数规则。格式化器不得借排版机会重排其它声明、改变表达式求值顺序、改写字面量值或删除显式丢值分号。
...
- `--check` 不写入源文件、权限、缓存、锁文件或 build.gg 生成物；文件读取的访问时间更新遵循文件系统的挂载策略。
...
规范格式的变更必须先更新本章并说明 formatter 输出变化、迁移规则和是否会改变诊断位置。格式化器版本不能写入 `gugu.toml` 或 `gugu.lock`；编译器构建身份仍按[包、依赖与构建模型](packages-builds.md)进入编译缓存 key。格式变化不得改变语言 ABI、package ID 或运行时语义，也不得改写已发布归档及其校验和；本地源码字节变化会正常改变下一次归档的内容校验和。
