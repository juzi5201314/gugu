...
移位量可以是任意内置整数类型，结果类型由左操作数决定；这不构成其它算术运算的隐式数值转换。除数非零、负移位量、范围边界、浮点转整数和 Unicode 标量有效性属于各自操作的检查，必须与已经按顺序求值的操作数关联，不能为检查再次求值。运行时必定触发 panic 的常量操作仍按运行时规则处理；comptime 中执行到该操作才产生编译期错误。
...
切片边界要求 `0 <= a <= b <= xs.len()`；省略起点取 0，省略终点取长度。
`xs[len..len]` 与 `xs[len..]` 都是合法空切片，不能套用单元素下标的严格小于检查。
...
## 返回、循环退出与 `defer` {#return-loop-exit-defer}
...
类型检查区分注册时读取与退出时读取：调用形式的参数必须在注册点已初始化；块形式读取捕获槽时，槽必须在每个实际执行该 action 的退出路径上已初始化。`defer ret` 在内层块注册后不会因该块正常结束而提前执行。defer 体内部的类型错误不因外层路径不返回而被忽略。

条件注册的块清理只合流实际注册该 action 的前驱；初始化状态与函数值来源一起保留。退出内层词法块后仍按原捕获槽查找函数值，注册之后的赋值在清理执行时生效，不能把其它未注册路径的闭包来源混入该 action。
