...
- **G**：有栈绿色协程。用户代码里的并发单位。数量目标：同一进程内百万级。
- **M**：OS 线程。真正执行机器码。
- **P**：逻辑处理器（绑定 TLAB、run queue、GC 本地状态）。M 必须持有 P 才跑 G。

不是 Rust/JS 的 `async fn`：函数没有颜色，没有 `.await` 传染。关键字 `async` **只用来启动新 G**（见下）。普通函数里直接 `ch.recv()` / `h.wait()` 就会挂起当前 G，M 去跑别的工作。

## 有栈协程

- 每个 G 有一块**连续**栈，初始很小，不够则**拷贝增长**（禁止分段栈热分裂）。
- 切换是 IR 原语（寄存器 + 栈指针）。
- 创建：`async 调用` 或 `async { 块 }`，见 [表达式](expressions.md)。得到 `Join[T]`。
- `yield`：主动让出。
- 进程何时等用户 G、何时杀掉，见 [运行时 · 进程寿命](runtime.md)。

## 抢占

不是纯协作。每个 G 连续运行超过时间片后必须被切走，即使它在算 CPU、没有主动 `yield`。

抽象机：

1. 调度器给正在跑的 G 一个时间量子。
2. 编译器保证 G 在有界的工作之后进入 **safepoint**（函数入口、循环回边、调用点）。无调用的热循环也必须有回边轮询。
3. 量子耗尽后，下一次 safepoint 切走该 G。
...
- 类型与构造：`chan[T]`、`chan[T](n)`。`n` 是缓冲长度，`n == 0` 即无缓冲。`chan` 是关键字，这种调用不是下标。
- 无缓冲：发送与接收会合。
...
一次等待多个 channel 的 `send`/`recv` 以及 `Join.wait()`，随机公平选择就绪分支，可有默认 `_`（不阻塞）。求值顺序与作为表达式的类型规则见 [表达式](expressions.md)。

## 内存序与数据竞争

多个 G 可以同时跑在多个 M 上。数据竞争是未定义行为。类型系统**不**静态禁止把 `&T` 送进另一个 G（没有 `Send`/`Sync` 约束）：GC 延命解决的是寿命，不是互斥。跨 G 共享可变状态是程序员义务，必须走 `chan`、`std.sync`、原子、或只读共享。编译器可以提供竞态检测器构建，但不保证检出全部数据竞争。
