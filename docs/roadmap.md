...
- [x] **阶段 46：实现 SharedHeap stable handle 与 forwarding grace**（复杂度：5）
  ...
  - 接入证据：world 侧闭环补齐——registry 增加按 descriptor 稠密索引的 block 记录（`used_bytes`/`live_bytes`/`dead_bytes`/`live_payloads`/`sealed`，切换填充 block 时给旧 block 封口）、按 handle 的在飞搬迁记录（`SharedPendingForward`）与 owner 扫描/交接接口；`forward_shared_payload` 串联 pin/状态门禁、registry 预留、`SharedHeap::forward`、登记项换址与 `HandleForward` 发布（单调非零 lease、`forward_generation + 1`、staging 目标改变先 flush）；`service_handle_forward` 由 payload owner 消费，依次校验目标 token、integrity、topology/cycle epoch、owner 归属、`new_payload` 与在飞记录，再结清 forwarding lease 并推进 4 步 grace；`settle_shared_forwards` 重试未结清记录；`run_shared_plane`（要求已开始的 mark cycle 与覆盖全部 owner 的 scope）做 settle、sweep（未标记 + 状态 live + 无旧 payload + 全部 lease 归零 + 无在飞搬迁 → `mark_returned` + `release` + 登记项回收）与 block 搬迁（`sealed && live_payloads > 0 && dead_bytes > 0 && live_bytes <= dead_bytes`，被 pin 的 payload 计入推迟），并在同一轮排空 inbox 让 grace 就地结清，接入 `pressure_drain` 第 7 步与 `PressureDrainReport` 的五个 `shared-*` 字段；`mark_termination` 的 forwarding work 计入在飞搬迁；`handover_managed_arenas` 同时转移 registry owner 与 card table manager（`BarrierPlane::handover_manager`），共享 block 的 owner/manager 解析按编号段分流（edge 路由、mark ticket 来源身份），候选回收域显式排除共享 block；世界 access guard 改为 token API（`begin/load_with/store_with/end`），一个 guard 覆盖同一 `SharedAccessBegin/End` 区间内的读写，旧 `load_shared_field`/`store_shared_field_value` 已删除。真实源码回归：`runtime/fixtures/shared_sender.gg`（闭包跨 channel 后仍被 sender 使用）由 `SharedForwardHarness`/`benches/shared_forward.rs` 驱动「分配 → 字段写入 → 搬迁 → 通知消费 → grace 结清 → 一轮 forced cycle」，`cargo bench -p gugu-compiler --bench shared_forward --profile dev` 实测 `forwards=512 bytes=2097152 freed=4194304 empty-blocks=127 invariants=true`（dev profile，含 16 次 owner 排空与 1 次完整 cycle，耗时不作为性能口径）；新增确定性回归 10 个（`runtime/world/shared_forward_tests.rs`）+ LIR 零共享站点 1 个 + CLI 零共享需求断言，`cargo nextest run --workspace` 由 799 增至 810 全绿。
  - 验收证据：guard 结束前旧 payload 有效由 `guard_keeps_old_payload_alive_until_end` 断言（搬迁 + 消费后 `payload_exists(old)` 仍为真、guard 内读到的仍是它当时解析的 payload、grace 停 `reclaimable`，`end_shared_access` + settle 后旧 payload 才消失）；direct pointer 不逃出 guard 由 LIR 侧 `shared_handle_storage_is_guard_wrapped`/`shared_environment_offset_stays_inside_guard` 与 guard 必须成对结束的 verifier 保证；重复与陈旧消息由 `handle_forward_rejections_leave_state_untouched` 覆盖五类拒绝（错误目标 owner、篡改 checksum、过期 handle generation、跳号 forward generation、消费后重放），每次都断言 lease/grace/在飞记录与失败前逐项相同；slot 状态与 forward generation 由 `pin_defers_forward_and_evacuation_skips_pinned_payloads` 断言（推迟不推进 generation、不改 current payload、不留在飞记录）与参照实现的状态迁移表校验；「共享访问额外成本不扩散到 LocalHeap」由 `local_heavy_world_stays_free_of_shared_state`（只有 LocalHeap 对象跑完整 forced cycle 后 registry 为空、handle slot 数为 0、累计计数与 `PressureDrainReport` 的 shared 字段全 0）、`lir::tests::local_only_source_has_no_shared_heap_sites`（需求全零且无任何共享站点）与 `gugu-cli` 的 `build_json_reports_shared_heap_contract_keys`（`shared-heap-demand` 11 个字段全 0，同时 `local-heap-demand.managed-types > 0`）三层共同覆盖；端到端 smoke：`GUGU_INTERNAL_OPTIONS=1 gugu build --format json -Zdump-runtime` 在共享夹具上退出码 0 且 `shared-heap-demand.alloc-sites = 1`，在只有 LocalHeap 的源码上 11 个字段全 0。

- [x] **阶段 47：实现 GC block return 与全链路 owner 回收**（复杂度：5）
  - 依赖：阶段 30–32、41–46。
  ...
  - 接入证据：`RuntimeRawModel`（query 30）升到 schema 19，并入 `BlockReturnRuntimeContract`（`BLOCK_RETURN_SCHEMA = 1`，profile `mosaic-block-return` revision 2）：四类 return unit（`heap-block`/`heap-line-run`/`heap-arena`/`large-mapping`）、八个发布门禁（`incoming-lease`/`allocator-lease`/`scanner-lease`/`evacuation-lease`/`pin`/`resource`/`live-lines`/`candidate-job`）、`HeapLineRun` 最短 8 line、queue-page grace 4 步；`BlockReturnDemand` 由 LocalHeap/SharedHeap 需求派生，缓存恢复后仍逐字段核对；managed 物理页进入独立 `ManagedAccounting`，账本按页的提交 owner 归属。
  - 验收证据：`runtime/world/block_return_tests.rs` 覆盖块归还、line-run 恢复、large mapping、pin 拒绝、grace 延迟 decommit、共享空块归还与账本守恒；`runtime/world/heap_tests.rs` 覆盖大对象 span 复用、待归还块不可分配、arena 摘除与 pacing live 基线；`cargo bench -p gugu-compiler --bench block_return` 报告 `invariants=true` 且至少归还一个 block。

- [x] **阶段 48：实现 checked pointer compression**（复杂度：4）
  - 依赖：阶段 46、47、38。
...
- [x] **阶段 49：实现 radix routing profile**（复杂度：4）
  - 依赖：阶段 30、35、47。
  ...
  - 接入证据：`RuntimeRawModel`（query 30）升到 schema 21，并入 `RoutingRuntimeContract`（`ROUTING_SCHEMA = 1`，profile `mosaic-routing` revision 1，raw policy revision 升到 3）：路由模式目录（`direct`/`radix`）、固定 `2^k` bucket（`RADIX_BUCKET_LOG2 = 6`，64 bucket）、有限 levels（`MAX_RADIX_LEVELS = 2`）、转发 hop 上限（`RADIX_HOP_LIMIT = 4`，verifier 要求不小于 levels）、radix 拦截的消息族目录（return 族）、maintenance 相位目录与两条切换序列（direct→radix 为 `freeze-target-cache`/`flush-staging`/`publish-mode`，radix→direct 为 `drain-buckets`/`drain-forwarding`/`publish-mode`/`restore-target-cache`，相位不可跳过、`drain-buckets`/`drain-forwarding` 要求在飞量清零才能推进）、五项统计口径（`remote-return-hops`/`radix-batches`/`maintenance-switches`/`old-topology-drained-batches`/`injection-fallbacks`）；`RoutingDemand`（owner 数、return 发布站点上界）由 raw 平面需求推导并与契约逐字段交叉校验，路由模式是 policy 的唯一来源（契约段与 policy 不一致即拒绝），policy 是 query key 首字段，模式变化自动失效缓存与 action key。`runtime/routing.rs` 的 `RoutingPlane` 是契约的确定性对偶：direct 模式 bucket 容量为 0（不分配任何 producer×owner 队列矩阵）；radix 模式按原始 target 的 route key bits 逐层 staging，每步恰好一跳（终层先解析、随后才降层），终层经 owner directory 解析为交付（保留原 integrity）、转发（旧 epoch 沿转发记录进入新 token）或 domain injection（owner retire/不可达终点），Unknown/route key 不符按原错误稳定传播；每跳计入 `remote-return-hops`，超 hop 上限为稳定 invariant。`world/routing_impl.rs` 把发布路径接到真实世界：radix 模式下 return 族的 staging flush（含 managed block return 的 `publish_message` 路径）与 ring 关闭批次进入平面，`drain_routing` 以固定迭代上界推进，交付/转发/注入由 world 执行——转发与注入按新终点重写 node 内容、重算 integrity、旧 node 进入 grace，与 `forward_message` 语义一致；`switch_routing_mode` 驱动完整相位序列（冻结期发布被拒绝、两种 mode 不同时消费同一 queue head、pending bytes 始终计入 credit 快照的 pending 口径），owner `begin_forwarding`/`retire` 接入 topology epoch 观测，旧 epoch 在飞 batch 最终进入新 token/domain。`ImagePlan`/CLI JSON 报告 `routing-contract-fingerprint`、`routing-demand`、`routing-runtime`、`routing-profile`、`routing-profile-revision`、`routing-mode`、`routing-bucket-count`、`routing-max-levels` 与 `routing-hop-limit`，dump 输出 `routing`/`routing-families`/`routing-phases`/`routing-switch`/`routing-statistics`/`routing-demand`/`routing-fingerprint` 七段；`CompileRequest::with_routing_policy` 显式选择模式并随 raw policy 进入契约与 action key。工作区 913 项测试全部通过（阶段 49 新增 16 项：契约自洽与漂移拒绝、模式参与契约身份与 action key、direct 不分配 bucket、bucket 路由确定性与 FIFO、hop 上限与错误 forward 拒绝、旧 topology 转发/注入计数与 epoch 回退拒绝、两条 maintenance 序列与相位门禁、world 端到端 exactly-once 交付/转发/切换/ring 路由、CLI JSON 键），fmt/build 零 warning，mdBook 构建通过；真实编译默认 mode=direct、冷热契约指纹一致。

- [x] **阶段 50：实现 raw link provenance 与 release 安全 profile**（复杂度：4）
  - 依赖：阶段 30、47、48、49。
  ...
  - 接入证据：`RuntimeRawModel`（query 30）升到 schema 22，并入 `ProvenanceRuntimeContract`（`PROVENANCE_SCHEMA = 1`，profile `mosaic-provenance` revision 1，raw policy revision 升到 4）：释放 provenance 检查目录（`canonical`/`alignment`/`range`/`class`/`owner`/`generation`/`link`/`state` 八项校验加 debug/security 机制检查 `poison`/`double-return-marker`/`full-chain`/`random-reuse`/`guard-region`/`checked-copy`）、per-domain secret 登记目录（与 `MemoryDomainId::ALL` 一致）与管理规则（per-domain derivation、非零、只存 non-moving metadata、初始化失败 fatal）、release 基线检查集合与 profile 激活序列（release 只保留基线；debug/security 各自追加三项并逐项校验在目录内）、七类释放拒绝分类（`chain-corruption`/`forged-link`/`double-return`/`cross-owner`/`cross-class`/`stale-generation`/`guard-region`）与六项统计口径；`ProvenanceDemand`（owner 数、class 数）由 raw 平面需求与两份 class ladder 推导，安全 profile 以 policy 为唯一来源，profile 变化使契约指纹与 action key 失效。`runtime/provenance.rs` 的 `ProvenancePlane` 是契约的确定性对偶：per-domain secret 经派生键 `gugu-provenance-domain-secret-v1` 从 world secret 推导且互异非零，每个 domain 持有独立 `LinkCodec`，encoded link 因此是 domain 绑定的（跨 domain 解码在 checksum/tag 层被拒）；`SlabTable::pop_free_detached` 把弹出深度参数化，链上每一跳都经过 encoded link 解码，深度越过链长收敛到链尾；`verify_free_chain_checked` 把链走查失败分类为 `ChainWalkError`，链损坏（checksum/空链/环）、伪造（范围/对齐/外来/越 span）与过期 generation 各归其类。`world/provenance_impl.rs` 把释放路径接到真实世界：`RawWorld::new` 以 release profile 建立平面，`configure_provenance` 按契约对齐 mode；释放拒绝在既有检查点按类记账——`queue_return`/`local_return` 的旧 generation 与 debug 双返还标记、`service` 的 integrity/owner/bytes/状态/资源 cell generation 检查、`verify_links` 的全链走查——状态机判定与失败文本保持不变，只补充分类统计；debug 的 poison 写入挂在返还路径、分配时校验标记未被改写并清除，security 的复用深度由平面内独立 seed 决定（`REUSE_DEPTH_LIMIT = 8`），`checked_copy` 校验读/写权限、双方越界与 guard region 重叠后才拷贝且失败不产生部分写入。`CompileRequest::with_provenance_policy` 显式选择 profile 并随 raw policy 进入契约与 action key；`ImagePlan`/CLI JSON 报告 `provenance-contract-fingerprint`、`provenance-demand`、`provenance-runtime`、`provenance-profile`、`provenance-profile-revision`、`provenance-mode`、`provenance-check-count`、`provenance-domain-count` 与 `provenance-rejection-category-count` 九键，`-Zdump-runtime` 输出 `provenance`/`provenance-checks`/`provenance-active`/`provenance-domains`/`provenance-secret-rules`/`provenance-rejections`/`provenance-statistics`/`provenance-demand`/`provenance-fingerprint` 九段。工作区 931 项测试全部通过（阶段 50 新增 18 项：契约自洽与漂移拒绝、profile 参与契约身份与 action key 且冷热一致、per-domain secret 互异与 domain 绑定、伪造 tag 的 Foreign→forged-link 分类、debug poison/双返还标记/free 期间写入检测/全链计数、security 随机复用偏离 LIFO 但集合与账本一致、guard region 与 checked copy 语义中立、五类释放拒绝的端到端分类），fmt/build 零 warning，clippy 无新增警告，mdBook 构建通过；真实编译默认 mode=release、冷热契约指纹一致。

- [x] **阶段 51：实现 typed combining 与 topology/range 慢路径**（复杂度：4）
  - 依赖：阶段 32、35、49、50。
  ...
  - 接入证据：`RuntimeRawModel`（query 30）升到 schema 23，并入 `CombiningRuntimeContract`（`COMBINING_SCHEMA = 1`，profile `mosaic-combining` revision 1，raw policy revision 升到 5）：冷操作 tag 目录（`global-range-refill`/`extent-coalesce`/`topology-rebuild`/`platform-trim`）、与 tag 目录逐项相同的允许用途与十项禁止用途（TLAB allocation、raw local pop、普通 remote return、channel/select/park-wake 线性化、任意 closure、drop glue、coroutine stack call、共享 free-list guard）、operation record 的十个标量字段（`tag`/`state`/`generation`/`owner-domain`/`owner-id`/`merge-key`/`descriptor`/`scalar`/`bytes`/`response-slot`，任何地址种类都被 verifier 拒绝）、状态目录与七条迁移（`recycle` 同时服务 completed 与 cancelled）、单条 fast path（`compare-exchange-claim-word`）与两条争用路径（`mcs-record`/`owner-inbox`）、wait/wake 平台接缝与九项统计口径，以及 `MERGE_LIMIT = 4`、轮次预算（8 条 / 1 MiB）、`TIMEOUT_ROUNDS = 8`、记录规范槽 64 B、chunk 16 槽（尾部 1 槽预留给 refill）与 256 chunk 上界；`CombiningDemand`（owner 数、分配站点数、extent class 数）由 raw 平面需求推导，verifier 拒绝需求与公式不符。`CombiningPlane` 是契约的确定性对偶：direct 模式不创建记录，combined 模式按 chunk 整块补充非移动记录池、无争用占用单原子 claim 字、争用挂到 owner 的 MCS 链尾，由固定上界的轮次按 FIFO 认领并按 `(tag, merge_key)` 合并执行；`world/combining_impl.rs` 把世界四条真实冷路径接进平面（平台 trim 的页撤销、extent coalescing 的 buddy 合、topology 目录稠密索引重建、记录池 `GlobalRange` refill），`service`/`drain_all` 的推进点只做 O(1) 空队列检查，`owner_slot` 与 `extent_owner_index` 改查同一张 topology 索引，`ImagePlan`/CLI JSON/`-Zdump-runtime` 暴露 `combining-*` 键与 dump 段；Gugu runtime 侧的等价实现随 rt0 与调度复用同一 schema 与 verifier。
  - 验收证据：`runtime/combining_tests.rs`（12 条）覆盖契约目录与指纹、漂移逐项拒绝、fast path 单原子认领与无遗留、争用挂链与下一轮认领、有限同类合并（8 条记录合成 3 次执行、合并省下 5 次）、轮次条目与字节预算（20 条只认领 8 条；半预算请求只认领 2 条）、取消只允许发生在认领之前、链上等待到超时轮数后以 `timed-out` 收尾、response 恰好一次与已回收句柄按 generation 拒绝、槽位 generation 推进、记录池按 chunk 非移动补充与 chunk 上限、池耗尽是不变量失败且 refill 可用预留槽位；`runtime/world/combining_tests.rs`（9 条）覆盖 direct/combined 双模式 trim 结果逐项一致（`trimmed`/`blocked`/`committed_bytes` 相同，direct 九项计数不变）、同 class 批量 trim 的合并算术（每条 extent 两条冷操作，`executions + merged == requests`）、热路径（分配/本地返还/远程归还/owner drain/pressure trim 门禁）九项计数守恒、coalescing 结果在 direct/combined 与重复运行之间逐项一致、topology 重建经平面执行且索引查询与线性扫描等价（伪造 route key 被同一错误拒绝，domain owner 独占最后一条队列）、争用请求在平台 wait 字登记等待并被下一轮唤醒（物理页只撤销一次）、31 条 extent 触发记录池 refill（`refills` 与 refill 请求数一致）、在飞记录存在时配置平面被拒绝且统计与在飞记录都不变、已取消的冷操作永不应用范围动作（extent 仍 Live、committed/decommitted 不变，parked 读回 cancelled/timed-out 均报「combining 请求未被执行」）；`crates/gugu-cli/src/main_tests.rs` 新增 1 条断言 10 个 `combining-*` 键、profile/revision/mode/tag 数/预算与 demand、指纹非零。`cargo nextest run --workspace` 953 项全通过；`cargo bench -p gugu-compiler --bench cold_combining` 在默认 `owners=2 extents-per-owner=16` 下报告 `requests=67 fast-path-claims=3 parked=64 merged=48 executions=19 rounds=8 refills=3 trimmed-extents=32 trimmed-bytes=2097152 hot-path-entries=0 invariants=true`（`GUGU_BENCH_EXTENTS=64` 时按 2 MiB arena 的 span 上界自适应到每 owner 32 条）；真实 CLI 在 `/tmp/gugu-combining-smoke` 上以默认 direct 输出 10 个 `combining-*` 键与 `combining schema=1 profile=mosaic-combining revision=1 mode=direct tags=4 …` dump 段。

## 五、x86_64 后端、镜像与外部边界

- [x] **阶段 52：实现 target descriptor、数值 lowering 与 x86 encoder**（复杂度：5）
  - 依赖：阶段 28、29、32。
  ...
  - 接入证据：`target.rs` 固定 `CpuFeature`/`CpuBaseline::X86_64V1`/`CPU_FEATURES` 目录、`TARGET_PAGE_SIZE`、`IMPORT_POLICY_REVISION`、`TARGET_SYSROOTS` 与 `TargetDescriptor::digest()`（域 `gugu-target-descriptor-v1`）、`BackendCostProfile::digest()`；`backend/x64/` 落地封闭指令表（412 个 form，其中 5 个登记为 baseline 之外：`pshufb`/`pmulld`/`pextrd`/`pinsrd`/VEX `vaddps`）、`encode.rs`（长度先行、前缀/REX/ModRM/SIB/disp/imm 逐字段写出）、`verify.rs`（baseline、lock/xchg、保留寄存器与寄存器纪律）、`contract.rs`（`ENCODER_SCHEMA`/`ENCODER_REVISION`/`LOWERING_REVISION` 与域 `gugu-x64-encoder-v1`/`gugu-x64-encoder-catalog-v1` 的目录指纹）、`codegen.rs`（`CODEGEN_SCHEMA`、逐实例 `QueryKind::CodegenFragment` 键、站点 payload 与片段/世界指纹）、`harness.rs`+`harness_cases.rs`（覆盖全部 lowering 规则的确定性用例与解码夹具）；`lower/{numeric,atomic,vector}.rs` 把 `IConst`/`FConst`/`Integer`/`Float`/`Compare`/`Convert`/`Vector`/`Select`/`TrapIf`/`Atomic`/`Fence`/`DecodeCompressedRef` 展成机器序列，`lir/pass/abi.rs::legalize_x86_64` 在 `lir::build` 捕获的 CPU 基线下运行并在同一处过 `verify_sequence`，没有基线序列的 operation（I64 排序比较、`V128` 乱序 select、浮点 `VReduceAdd`）是编译错误而非回退路径；`lir/effects.rs::poll_cost_with` 让机器化 operation 取 `x64::lower::poll_cost` 的机器形式权重和（`POLL_COST_REVISION` 1→2），strip-mine 尺寸与预算用同一口径；`DiagnosticCode::BackendInvariant`（E0060）与 `ActionKind::Codegen`、`project/action_key.rs` 的 target/encoder/fragment 输入、`ImagePlan` 的 14 个 x64 键、CLI `-Zdump-x64` 与 `x64-dump` JSON 事件把结果接到编译流水线；runtime 侧新增 `runtime/cage_control.rs`（`CageControlRecord`/`CAGE_CONTROL_FIELDS`/`CAGE_CANONICAL_LIMIT`）与 `runtime/harness.rs::DecodeReferenceFixture`，`CompressionRuntimeContract` 随之升到 `COMPRESSION_SCHEMA = 2`、`RuntimeRawModel` 升到 schema 24。
  - 验收证据：`crates/gugu-compiler/src/backend/x64/tests.rs` 的 byte-level fixture 逐字段核对 relocation/addend、address mode、scale/disp8、原子 lock 与约束偏移，`encoder_contract_matches_descriptor_table` 与 `encoder_contract_rejects_tampered_catalog` 校验目录与篡改拒绝，`sse2_integer_forms_carry_operand_size_prefix` 与 `fence_forms_emit_fixed_modrm` 把「66 前缀」和「固定 ModRM」两类只在执行期暴露的错误钉在表级；`cargo nextest run --workspace` 994 条全绿；`cargo bench --bench x64_encoding` 在真实宿主 VM 上执行 261 条用例（`cases=261 matched=261`），逐例比对寄存器/内存/冷边，并用同一路径核对解码序列的 `decodes=1`/`rejections=4` 与 `CompressionPlane` 逐个吻合、`switch-ok=true`（编码器生成的上下文切换片段与 `ContextSwitchCode::fixed()` 逐字节相同、restore offset 一致）、`invariants=true`、`forms=412`、`beyond-baseline=5`；该执行路径先后发现并修掉 SSE2 形式缺 `66` 前缀（落到 MMX）、`mfence` 的 opcode 与无操作数形式的固定 ModRM 未发射、进位链用 `adc`/`sbb` 重复计入 CF、浮点向量比较误用「无序为真」谓词、插入掩码左移量错、32 位 lane 归约丢位等归属层缺陷；`cargo bench --bench coroutine_context` 用 runtime 的真实换栈片段跑通往返恢复与寄存器哨兵。

- [x] **阶段 53：实现 instruction selection、内部 ABI 与 block layout**（复杂度：5）
  - 依赖：阶段 52、28。
  ...
  - 接入证据：`processor::LogicalProcessorPrefix` 发布 poll/ownership/TLAB/TurnRegion 的 `offset_of!` 结果，`SchedulerRuntimeContract` schema 2 与 `RAW_MODEL_SCHEMA = 25` 冻结这些偏移；编码器 `ENCODER_REVISION = 2` 增加 `Rel8` 与 17 个短跳 form；`backend/x64/{abi,mangle,layout,copies,select}.rs` 与 `lower/{memory,call,ctrl,alloc,asm}.rs` 对封闭 LIR opcode 穷尽 lowering：TLAB/TurnRegion bump 用置标志位的 `add` 做进位检查并按 payload 对齐、块参数拷贝按**边**发射并在选指阶段用临时虚拟寄存器打断环、rel8 收缩按缩短后的指令终点判定；内部符号 `__gugu_<kind>_<64 hex>` 且 key 与 mangled 文本同源；`CODEGEN_SCHEMA = 3` 的 fragment 携带 symbol/符号集合/rel8/hot/cold/ABI 槽，query key 并入 scheduler/coroutine/universe 指纹；CLI JSON 暴露 `x64-rel8-count`、`x64-hot-block-count`、`x64-cold-block-count`、`x64-entry-symbol`（`RootCategoryV1::Entry` 实例的片段，不按片段顺序推断）、`scheduler-poll-flags-offset`。
  - 验收证据：`select_tests` 覆盖内部整数/浮点槽、sret 指针占用第一个整数槽及其与寄存器结果互斥、>16 字节间接聚合、Win64 位置槽与 shadow space、栈参数的 outgoing 偏移、间接目标不占参数槽、vtable 两种形态（`Metadata` lane 与胖对 `+8`）及其机器序列、`r14` 禁止写入、mangling 长度与符号 key 同源、`NoSafepointBegin` 零字节、Relaxed load 无 mfence、AcqRel CAS 的 lock cmpxchg、前向/反向 rel8 边界与收缩后的稳定计数、拷贝环（2 元/3 元/无环链）不丢源、GcAlloc bump 的进位来自 `add` 且含对齐掩码；`lower::tests` 覆盖数值/向量/原子/解码；`simple_main_reaches_runtime_and_image_plan` 断言 `x64-entry-symbol` 的实例键等于 LIR dump 里 `fn main instance=<hex>`（片段按实例键升序，按顺序推断会错）；CLI `build_json_reports_x64_fragment_keys` 要求入口 `__gugu_fn_` + 64 hex、dump `x64 schema=3` 与分支助记符；`cargo bench --bench x64_select` 编译含分支的 main 并打印 bytes/rel8/hot/cold/symbol。

- [x] **阶段 54：实现线性扫描寄存器分配与固定 frame**（复杂度：5）
  - 依赖：阶段 53、29。
  ...
  - 接入证据：`backend/x64/alloc/{live,scan,slots,frame,parallel,rewrite,mod}.rs` 在选指序列上做全局线性扫描：position 用 `use_slot = 2p`/`def_slot = 2p + 1`，每个点位携带 `kind`（`Normal`/`Call`/`Bridge`/`Prologue`）与**读也计入**的 mask（Call 加九个 caller-saved GPR 与全部 XMM）；候选寄存器必须不在覆盖范围内的 mask、未被占用且通过 `r/m8` 宽度检查，缺候选时按 `spill_weight / remaining_length`（`u128` 交叉乘法、相等时 spill value ID 较大者）淘汰，跨 Call/Bridge 的值优先 `rbp`/`r12`/`r13`，managed/stack 指针按根类强制落栈。`CODEGEN_SCHEMA = 4`、`LOWERING_REVISION = 3`、`ALLOCATION_REVISION = 1`（进入 fragment key）：片段 payload 新增 `frame`/`points`/`values`/`stats` 与站点级 `points` 范围，逐站点产出 `frame_size`/`payload_bytes`/`outgoing_bytes`/`required_frame`/`locals`/`spill_slots`/`scratch_offset`/`save_offsets`/`checked` 与 spill/reload/rematerialization/copy 统计；`verify_allocated_sequence` 拒绝任何残留虚拟寄存器、只允许 ABI 区间写 `rsp`、禁止 `r14`/`r15`。
  - 验收证据：`alloc_tests` 断言逐片段 `payload_bytes <= frame_size`、溢出槽字节装得进 payload、save 集不超过 `rbp`/`r12`/`r13`、copy scratch 落在 payload 内、`required_frame >= frame_size`、峰值活跃 GPR 不超过 12 个池大小，以及重编译的 fingerprint 与 dump 完全一致；`addressed_local_survives_a_call` 覆盖 `StackAddr` 跨调用的分配。`cargo bench --bench x64_frame` 把片段按符号映射为可执行页、解析内部 relocation 后用 `ret` stub 兜住运行时入口，在真实 CPU 上按内部 ABI 调用入口片段：无调用流水线、纯算术 while 循环、带溢出的两次 6 参数 `mix` 循环与 `spare(total) + mix(...)` 四个场景逐个与宿主参照实现比对捕获值，并断言没有执行到未知 stub。该 bench 发现了三处只有在执行期才可见的缺陷并修在归属层：`Switch` 默认边（otherwise）的块参数拷贝从未发射、终结符内跳板标签的定义位置没有按函数序列重定基、`load`/`store` 对 `F32`/`F64`/`V128` 误用整数 `mov`（`movsd`/`movss`/`movups` 缺失）。

- [x] **阶段 55：接入 stack map、panic landing 与 unwind table**（复杂度：5）
  - 依赖：阶段 38、54。
  ...
  - 接入证据：`CODEGEN_SCHEMA = 5`，`METADATA_SCHEMA = 1` 进入 fragment key。分配与编码之后由 `backend/x64/metadata` 生成函数内 safepoint、landing 与 source record；`metadata_section::assemble` 按 mangled 符号升序、16 字节对齐分配逻辑 `code_rva`，`unwind_index` 等于该序号。栈图节沿用 `GUGUSM01` / version 2（Linux `.gugu.stackmap`，Windows `.gugustk`），map 由 `stackmap_codec::encode` 按完整记录字节字典序去重。统一展开表魔数 `GUGUUN01`、版本 1、逻辑节 `.gugu.unwind`（header 32 字节，函数记录 32 字节，landing 16 字节，flags bit0 panic landing、bit1 runtime bridge）。源码表魔数 `GUGUSR01`、版本 1，32 字节 `SourceRecord` 落入 `.gugu.meta`。零 frame 且无 safepoint、无 landing 的叶不进入表。`CallReturn` 只登记跨调用仍活跃的 `rbp`/`r12`/`r13` 根与传出栈参数；suspend/bridge 的跨点寄存器根被拒绝；`MorestackEntry` 的 `slot_count` 为 0。可重物化的 `StackAddr` 不记成栈内根。三节指纹域 `gugu-x64-metadata-v1` 进入世界指纹。装配调用 `frame_walk::audit`，并按每条 safepoint/landing 回读 `safepoint_at`/`landing_at`。strip 必须保留栈图节、`.gugu.unwind` 与 `.gugu.meta`。codegen action、镜像计划与 CLI JSON 暴露 `x64-metadata-schema`、`x64-stackmap-section`、`x64-stackmap-bytes`、`x64-stackmap-functions`、`x64-stackmap-safepoints`、`x64-stackmap-maps`、`x64-unwind-functions`、`x64-unwind-landings`、`x64-source-records` 与 `x64-metadata-fingerprint`；`-Zdump-x64` 打印同一组计数。
  - 验收证据：`metadata_tests` 覆盖 Linux/Windows 节名与 strip 保留/拒绝、`stackmap.gg` 的 map 去重（map 数小于 safepoint 数）、重编译指纹稳定、`CallReturn`/`SuspendResume`、landing 的 `cleanup_chain` 与 `landing_pc` 经 `landing_at` 回读，以及 `poll.gg` 的 `PollResume`/`MorestackEntry`、`extern "C"` bridge 的 `ForeignBridge`、跨调用指针的 `copy_allowed` 与帧对齐。CLI `build_json_reports_x64_fragment_keys` 要求 `x64 schema=5` 与 `x64-metadata schema=1 section=.gugu.stackmap`。`cargo fmt --all --check`、`cargo build --workspace`（零 warning）、`cargo nextest run --workspace`（1026 passed）与 `mdbook build -d target/book` 均通过。
...
- [x] **阶段 56：实现 ELF64 static PIE 与 Linux rt0 写出**（复杂度：5）
  ...
  - 接入证据：`ELF_SCHEMA = 1`，指纹域 `gugu-elf64-pie-v1`，不改变 `CODEGEN_SCHEMA`。`backend/x64/elf` 写出 `ET_DYN` static PIE：虚址等于文件偏移，页 4096，程序头虚址 64，load bias 是 `AT_PHDR - 64`。首个 `PT_LOAD` 从 0 覆盖到只读段。纳入栈图的函数按 mangled 符号升序、16 字节对齐排在 `.text`（`0x1000`）之首，因此 `code_rva` 等于相对 `.text` 的偏移。其后是其余用户函数、按源名排序的运行时入口、`__gugu_trap`（`ud2`）、归档成员、恰好 6 字节的 PLT 与 `_start`。只读段含可选 `.interp`、哨兵字节、相对重定位表（`u64` 计数加 `{offset, addend}`）、常量与 `.gugu.stackmap` / `.gugu.unwind` / `.gugu.meta`。片段里的 `Symbol::Data(index)` 仍按函数内序号编码；写出改按字节内容寻址，域 `gugu-const-bytes-v1`，相同字节合并。类型描述符、类型号、vtable、类型节基址和 global 是 8 字节对齐的只读占位。`.data.rel.ro` 恰好一页并由 `PT_GNU_RELRO` 覆盖；BSS 是 filesz 为 0 的独立 RW `PT_LOAD`，不在 RELRO 下。`PT_GNU_STACK` 不可执行。没有动态导入时不写 `PT_INTERP`。未知 `__gugu_runtime_*` / `__gugu_glue_*` 一次报出全部名字。`encode_elf` 在哈希前用 bias 0 重放相对重定位，核对哨兵字节为 1，并拒绝重复写入。rt0 读取 `AT_PHDR`、只在 RELRO 范围内做自重定位、`mprotect` 为只读，再映射 8MiB `PROT_NONE` 栈并只把顶端 4KiB 改成可写。栈检查初值落在映射顶端，所以第一条带检查的序言调用 `morestack_or_poll` 向下提交 64KiB。`panic` 按 `{ptr, len}` 写出后 `exit_group(101)`。通道容量槽环的关闭字在 `+4088`：`channel_send` 与重复 `channel_close` 以 101 退出；`channel_try_send` 返回 `Ok=0` / `Full=1` / `Closed=0x101`；`channel_try_recv` 把机器字放在 `rbx`。`gc_alloc_slow` 与 `gc_region_slow` 返回新映射的载荷。`coroutine_switch` 是 `ContextSwitchCode::fixed()` 的 62 字节。`spawn` 记录代码与环境，`join_wait` 在首次等待时调用它。平台 `reserve_aligned` 返回页头地址，`commit` / `entropy` / `wait` 按内部 ABI 做 `mmap`、`getrandom` 或字比较。成功路径不使用立即 `exit_group`。镜像计划与 CLI JSON 暴露 `elf-schema`、`elf-type`、`elf-entry`、`elf-byte-count`、`elf-load-segments`、`elf-relative-relocs`、`elf-interp` 与 `elf-fingerprint`；指纹进入 action key。Windows 的 `EmitImage` 仍跳过。
  - 验收证据：`elf::tests` 覆盖 ET_DYN、四段装载、RELRO、无解释器、动态导入的 `PT_INTERP` 对齐 1、归档抽取、W+X 与位移溢出拒绝、未知运行时符号、纳入函数顺序、bias `0x400000` 只应用一次，以及 `coroutine_switch` 与固定片段逐字节相同。`cargo bench --bench elf_pie` 在 Linux 上启动四份 33426 字节镜像：hello 退出 0，panic 退出 101 且标准输出含 `bug`，捕获闭包与 `chan[int](1)` 的发送/接收都退出 0。`cargo fmt --all --check`、`cargo build --workspace`（零 warning）、`cargo nextest run --workspace`（1034 passed）与 `mdbook build -d target/book` 均通过。

- [x] **阶段 57：实现 PE32+、Windows rt0 与导入导出**（复杂度：5）
  - 依赖：阶段 52–55、阶段 33。
  ...
  - 接入证据：`PE_SCHEMA = 1`，指纹域 `gugu-pe32-plus-v1`，不改变 `CODEGEN_SCHEMA`。`backend/x64/pe` 写出 PE32+：image base `0x140000000`，节对齐 4096，文件对齐 512，时间戳与 checksum 为 0，subsystem `WINDOWS_CUI`，`DllCharacteristics = 0x0160`。`.text` 从 RVA `0x1000` 起，纳入函数的顺序与 Linux 相同，因此栈图 `code_rva` 仍是相对 `.text` 的偏移。常量继续按 `gugu-const-bytes-v1` 合并。节为 `.text`、`.rdata`、`.gugustk`、`.ggmeta`、`.xdata`、`.pdata`、`.reloc` 与一页 `.bss`。IAT 只含排序后的 `kernel32.dll`（`GetStdHandle`、`VirtualAlloc`、`VirtualFree`、`VirtualProtect`、`WriteFile`）和 `ntdll.dll`（`RtlExitUserProcess`、`RtlRandomEx`）。调用是 RIP 相对 `ff 15`，镜像中没有 `syscall`。未登记 C 符号报 `未登记导入`。没有 `Abs64` 时 base relocation 是一个只含 `IMAGE_REL_BASED_ABSOLUTE` 填充项的合法块。`cdylib` 设置 `IMAGE_FILE_DLL` 且 export directory 为空。`staticlib` 是时间戳为 0 的 COFF archive，成员为同一份 `.text`。`_start` 用 `VirtualAlloc` / `VirtualProtect` 建立栈、协程、处理器、TLAB 与 turn region，栈检查初值在映射顶端，再调用用户入口，经 `RtlExitUserProcess` 退出。不注册 console handler，`UNWIND_INFO` flags 为 0。用户函数在 frame 非 0 且找得到第一条 `sub rsp` 时写 `UWOP_ALLOC_*`，prolog 取该指令结束偏移；运行时入口用 0 条 unwind code。导入调用留 32 字节 shadow。`platform_entropy` 使用已登记的 `RtlRandomEx`，因为登记表里没有 `getrandom`。通道、拷贝、`wide_div` 与 `coroutine_switch` 的内部 ABI 与 Linux 写出相同。镜像计划与 CLI JSON 暴露 `pe-schema`、`pe-entry`、`pe-byte-count`、`pe-sections`、`pe-imports`、`pe-relocs`、`pe-fingerprint` 与 `pe-staticlib-bytes`；PE 指纹紧跟 ELF 指纹进入 action key。Windows 嵌套 `i128` 的 C ABI 仍是既有编译错误。
  - 验收证据：`pe::tests` 覆盖 MZ/PE、可选头魔数 `0x20b`、时间戳与 checksum 0、subsystem 3、`DllCharacteristics 0x0160`、image base、无 `syscall`、导入名只有 `kernel32.dll` 与 `ntdll.dll`、无 `msvcrt.dll`、`.text` 不可写、`cdylib` 的 DLL 位与空导出目录、非空重定位目录，以及 `!<arch>\n` 静态库。`configured_target_changes_raw_contract_fingerprint` 要求 Windows `emit-image` 为 complete、`pe-schema = 1` 且 ELF 字段仍为空。`cargo fmt --all --check`、`cargo build --workspace`（零 warning）、`cargo nextest run --workspace`（1037 passed）与 `mdbook build -d target/book` 均通过。本环境不启动 PE。

- [x] **阶段 58：实现 FFI bridge、foreign effect、asm 与 OS 适配**（复杂度：5）
  ...
  - 接入证据：`BRIDGE_SCHEMA = 1`，指纹域 `gugu-bridge-v1`，`RAW_MODEL_SCHEMA` 升到 26。规范化字节是 schema、阻塞额度、dirty 槽和 `normal\0dirty\0leaf\0`，指纹不编码自身。`BridgeContract::fixed` 把阻塞额度定为 8、dirty 槽定为 1，两个目标共用这一段，因此 `bridge-fingerprint` 相同，整份 raw 指纹仍随目标语义变化并进入 action key。参照状态机不创建操作系统线程，按调用发序号：普通 bridge 取得额度后保留可取回的 processor lease 并登记桥帧根；dirty 立即释放 processor、占用唯一槽并登记桥帧根；leaf 留在当前 processor 上，不建桥帧根，也不允许回调。已有同类等待者时新调用排到队尾，不插队，也不计入 native。额度归还后只由 `resume_waiter` 让队首进入 native。普通 bridge 与 dirty 在 `finish` 前必须捕获 `errno` 与 last-error，重复捕获失败；leaf 结束时不强制捕获。未 pin 的指针不能交出。`native_stack_mapped` 与 `scans_native_unwind` 对已进入 native 的调用恒为 false。`register_external` 之后触碰协程或 GC metadata 失败。阻塞 poller 走同一套普通 bridge admission。调用效应、managed asm 的有限 CFG、`CString` 词法与栈图里的 `ForeignBridge` 记录沿用既有 lowering，不另写一套 ABI。镜像计划与 CLI JSON 暴露 `bridge-schema`、`bridge-max-blocking`、`bridge-dirty-slots` 与 `bridge-fingerprint`。
  - 验收证据：`runtime::bridge::tests` 覆盖契约指纹稳定、leaf 保留 processor 且拒绝回调与 lease 取回、第二条 dirty 进入等待且 native 数仍为 1、八次普通 bridge 之后第九次等待且归还额度后由队首恢复、捕获前不可读错误、pin 门禁，以及外部线程不能触碰 managed 状态。`configured_target_changes_raw_contract_fingerprint` 要求两个目标的 `bridge-schema = 1`、额度 8、dirty 槽 1 且桥接指纹相同。CLI JSON 含上述四个键。`cargo fmt --all --check`、`cargo build --workspace`（零 warning）、`cargo nextest run --workspace`（1044 passed）与 `mdbook build -d target/book` 均通过。
