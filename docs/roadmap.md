...
- [x] **阶段 53：实现 instruction selection、内部 ABI 与 block layout**（复杂度：5）
  ...
  - 接入证据：`processor::LogicalProcessorPrefix` 发布 poll/ownership/TLAB/TurnRegion 的 `offset_of!` 结果，`SchedulerRuntimeContract` schema 2 与 `RAW_MODEL_SCHEMA = 25` 冻结这些偏移；编码器 `ENCODER_REVISION = 2` 增加 `Rel8` 与 17 个短跳 form；`backend/x64/{abi,mangle,layout,select}.rs` 与 `lower/{memory,call,ctrl,alloc,asm}.rs` 对封闭 LIR opcode 穷尽 lowering，函数级序列先 rel32 再只许长变短收缩为 rel8；内部符号 `__gugu_<kind>_<64 hex>`；`CODEGEN_SCHEMA = 2` 的 fragment 携带 symbol/rel8/hot/cold/ABI 槽，query key 并入 scheduler/coroutine/universe 指纹；CLI JSON 暴露 `x64-rel8-count`、`x64-hot-block-count`、`x64-cold-block-count`、`x64-entry-symbol`、`scheduler-poll-flags-offset`。
  - 验收证据：`select_tests` 覆盖内部整数/浮点槽、sret、>16 字节间接聚合、`r14` 禁止写入、mangling 长度、`NoSafepointBegin` 零字节、Relaxed load 无 mfence、AcqRel CAS 的 lock cmpxchg、rel8 收缩；`lower::tests` 覆盖数值/向量/原子/解码；CLI `build_json_reports_x64_fragment_keys` 要求入口 `__gugu_fn_` + 64 hex、dump `x64 schema=2` 与分支助记符；`cargo bench --bench x64_select` 编译含分支的 main 并打印 bytes/rel8/hot/cold/symbol。
