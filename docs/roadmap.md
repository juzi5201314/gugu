...
- [x] **阶段 57：实现 PE32+、Windows rt0 与导入导出**（复杂度：5）
  ...
  - 接入证据：`backend/x64/pe.rs` 在 `PlanBackend` 里把已编码片段、栈图、展开、源码记录和 GC 节收成 Windows PE32+。image base 为 `0x140000000`，section alignment 4096，file alignment 512，COFF timestamp 与 checksum 为 0，subsystem 为 `WINDOWS_CUI`。`DllCharacteristics` 带 dynamic base、high-entropy VA 与 NX。`.text` 含 rt0、console handler、abort stub 和按符号名 16 字节对齐的用户片段；登记名不超过 8 字节的只读逻辑节各写一节；`.rdata` 放导入描述符、128 字节 `0xFF` 栈界限和 4096 字节 `0xFF` 处理器前缀；`.data` 只放 IAT；`.pdata` 与 `.reloc` 随后。`.pdata` 的 `RUNTIME_FUNCTION` 取自 `GUGUUN01` 平台尾，OS begin/end 为片段区 RVA 加上容器内相对 `code_rva`。导入表只含按名排序的 `kernel32.dll`（`ExitProcess`、`SetConsoleCtrlHandler`）和 `ntdll.dll`（`NtTerminateProcess`），不写 `msvcrt`、不扫描 syscall 号、不搜索宿主 DLL。rt0 第一条指令是 `sub rsp, 0x28`，用 RIP 相对地址安装只调用 `NtTerminateProcess` 的 console handler，再调用入口，最后 `ExitProcess(0)`。handler 不进入用户代码。`r14` 指向栈界限，`r15` 指向处理器前缀。绝对地址进入 `IMAGE_REL_BASED_DIR64`；`PcRel32` 与 `Rva32` 在写出前修完。重复槽、未知种类、可写可执行节和缺失入口在写出前失败。未解析的 `morestack_or_poll` 接到 `ret`，其余未解析符号接到 `ExitProcess(1)` abort stub。编译路径没有登记的 C 导出，导出目录为空。`cdylib` 置 `IMAGE_FILE_DLL`；`staticlib` 使用确定性 COFF archive（timestamp/uid/gid 为 0）。Linux 计划的 Windows 镜像为空，Windows 计划的 Linux 镜像为空。`ImagePlan` 与 CLI JSON 暴露 `windows-image-kind`、字节数、入口 RVA、重定位数、导入 DLL 数与指纹。栈图 `code_rva` 仍相对片段区。
  - 验收证据：`pe_tests` 覆盖 PE32+ 头、ASLR/NX、shadow space、无 CRT、无 `syscall` 立即数、元数据魔数、`cdylib` 的 `IMAGE_FILE_DLL`、重复槽、缺失入口、未知重定位、可写可执行拒绝，以及 COFF archive 按名抽取。`compiled_windows_main_is_an_exe_and_linux_image_is_absent` 对 `fn main() {}` 写出 exe 且 Linux 镜像为空。`compiled_main_is_static_pie_without_interpreter` 断言 Windows 镜像为空。既有 `windows_c_signature_rejects_nested_128_bit_values` 仍拒绝 Windows 上含 `i128` 的 C 签名。本机不能启动 PE，不声称 Windows 内核加载，也不把 abort stub 说成完整 SEH personality。`cargo nextest run --workspace` 1036 项全通过。
...
- [x] **阶段 58：实现 FFI bridge、foreign effect、asm 与 OS 适配**（复杂度：5）
  ...
  - 接入证据：`runtime/foreign.rs` 消费协程控制块既有的 lifecycle 转换，在 `RuntimeRawModel` schema 26 的 `ForeignRuntimeContract` 上实现普通 bridge、dirty admission 与 leaf。普通调用先占 `BridgeCredit`，worker 上限 8；没有额度时进入 `Waiting` 并放开 processor，不执行 native。dirty 调用立即置 `FOREIGN_DETACHED`，target 复用 `dirty_target`：`parallelism = 1` 时为 1，否则为 `parallelism - 1`；降低并行度不强杀已运行的 dirty work。attached lease 只在 GC、回调或退役时立刻打破；runnable 压力要过 20µs 逻辑宽限。native 返回前写入 errno/last-error。`ForeignLeaf` 保持 `Running`。未 pin 的受管根在进入 native 前被拒绝。只有普通 bridge 能回调，回调 panic 停在边界，不把协程改成展开状态。opaque dirty body 拒绝补写 stack map。已登记的外部线程不能操作协程或 GC metadata。Linux `epoll` 与 Windows `iocp` 只接收 socket 类完成事件；普通文件和 foreign 调用走 BlockingBridge。C 字符串的终止 NUL 只有 `cstring::terminate` / `scan` 一条路径，字面量与 LIR 都走它。`CallKind` 与 GIR 的 asm / `global_asm` 计数进入契约，并由 `ImagePlan` 与 CLI JSON 暴露 `foreign-max-blocking-workers`、三类站点数、指纹和 `foreign-runtime`。
  - 验收证据：`foreign_tests` 覆盖 lease 与宽限、credit 排队与一次归还、dirty 额度与不伪造 stack map、leaf 不回调、pin、外部线程拒绝、poller 分流，以及 C 字符串内部 NUL。`compiled_foreign_sites_enter_the_runtime_contract` 编译三条 `extern "C"` 调用后，普通、dirty、leaf 站点各为 1，指纹稳定。参照模型不创建真实 OS 线程，也不把 `std.ffi` 的方法体提前写成标准库。`cargo nextest run --workspace` 1045 项全通过。
