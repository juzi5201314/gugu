...
- [x] **阶段 38：实现 stack map、unwind 与可复制栈扫描**（复杂度：5）
  ...
  - 验收：CallReturn/suspend/bridge/stop 点的 managed roots 可精确枚举并更新；旧栈 opaque pointer 不会重用；缺 map、版本错误、范围越界和寄存器冲突在镜像写出前失败；第五类 `StackInterior` 寄存器掩码在固定 16 字节 map header 中往返保真。
...
- [x] **阶段 39：实现 Mosaic GC 元数据与精确 trace**（复杂度：5）
  ...
  - 验收：canonical ULEB128、唯一 END、size/align/TypeId、section offset、root range、vtable 和 glue 关系全部校验；`.gugu.types`/`.gugutyp` 与 `.gugu.meta`/`.ggmeta` 携带真实类型表、trace/value program 与 root/vtable/source/alloc 记录并进入 `ImagePlan`，collector 不对 runtime raw memory 做保守扫描。
  - 接入证据：`RuntimeRawModel`（query 30）升到 schema 10，在同一 `RuntimeRawContractV1` 中并入 `GcMetadataRuntimeContract` schema 1；前端从冻结 `TypeUniverse`、具体 GIR layout 和 HIR source 生成真实 `GcMetadataWorldV1`，编码并验证 `.gugu.types`/`.gugutyp`（`GUGUTY01`）与 `.gugu.meta`/`.ggmeta`（`GUGUMT01`）。真实 type/meta section 字节沿 `RawModelInputs`、runtime raw contract、backend plan 进入 `ImagePlan`；entry 的 trace/value offset 与 length、direct/interior/repeat/switch、COW/resource actions、root/source/alloc/vtable/glue 记录和 section fingerprint 均进入契约与 query key。`ImagePlan`/CLI JSON 报告 section 字节数和 fingerprint，dump 输出 section 行；回归覆盖第五类栈图寄存器根、section header/range 损坏、program 长度漂移、冷/热 section 一致性。
