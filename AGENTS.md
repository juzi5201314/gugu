...
- 实现对照现行 `docs/src/spec/` 与 `docs/src/internals/` 契约；发现规范或 internals 缺口时当场修订对应章节。
...
- 被其它页面用 `](#锚点)` 引用的标题必须写成 `## 标题 {#kebab-id}` 显式锚点；id 用小写英文短名，标题正文可以自由改写。mdBook 的自动 slug 会静默吞掉顿号、反引号和半角括号，且随版本变化，不能作为跨文件引用契约。`crates/gugu-compiler/tests/docs_links.rs` 强制这两条约束。
- 指向源码文件的相对链接（如 `../../../crates/...`）不进 book，不归锚点规则管。
