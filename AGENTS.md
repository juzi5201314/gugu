...
- 实现对照现行 `docs/src/spec/` 与 `docs/src/internals/` 契约，不对照 `docs/roadmap.md` 的阶段编号。发现规范缺口时当场修订规范；禁止把「阶段 N」写入 spec、internals、ADR、代码注释或用户可见输出。路线图阶段号只是临时任务标识，完成后从路线图删除，不是永久引用。
