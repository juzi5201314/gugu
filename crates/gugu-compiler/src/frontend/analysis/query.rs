...
use crate::frontend::hir::Validated;
use crate::frontend::semantics::CheckedSemantics;
...
pub(crate) fn run_world(
    ...
    let result = queries.compute(key, |context| {
        ...
        let mut world = solver::analyze(hir.module(), checked, early, model, policy);
...
pub(crate) fn input_fingerprint(
    ...
    hash.update(&checked.input_fingerprint);
    hash.update(&early.registry_summary);
