...
impl ActionInputs {
    ...
    /// 设置 generic GIR 世界指纹。
    pub fn set_generic_gir(&mut self, fingerprint: [u8; 32]) {
    ...
    /// 设置目标描述符指纹。
    pub fn set_target_descriptor(&mut self, fingerprint: [u8; 32]) {
        self.target_descriptor = fingerprint;
    }

    /// 设置 x64 encoder 契约指纹。
    pub fn set_backend_encoder(&mut self, fingerprint: [u8; 32]) {
        self.backend_encoder = fingerprint;
    }

    /// 设置机器码片段世界指纹。
    pub fn set_backend_fragments(&mut self, fingerprint: [u8; 32]) {
    ...
    /// 设置已写出 PE 镜像的指纹；未写出时保持全零。
    pub fn set_pe_image(&mut self, fingerprint: [u8; 32]) {
        self.pe_image = fingerprint;
    }
    ...
    pub fn key(&self) -> ActionKey {
        ...
        encode_bytes(&mut canonical, &self.pe_image);
