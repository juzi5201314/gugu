...
//!   用例字节末尾由宿主补 `ret`；`cold_edges` 里的 `rel32` 由宿主补 stub（stub 记一次进入）；
...
use super::table::{self, Access, OperandKind};
use crate::lir::body::{Op, Type, ValueType};
use crate::runtime::cage_control::{CAGE_CONTROL_FIELDS, CageControlRecord};
use crate::runtime::gc_metadata_contract::GC_ARENA_BYTES;
use crate::runtime::{CompressionPolicyV1, CoroutineContext};
...
impl std::fmt::Display for X64HarnessError {
    fn fmt(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::Verification { case, detail } => write!(formatter, "{case} 的序列非法：{detail}"),
...
/// 16..31 = `xmm0`..`xmm15`。宿主填寄存器时跳过 `rsp`（编号 4）。
...
pub struct X64Case {
    ...
    pub expected: u128,
    /// 其余结果寄存器的期望值：`(寄存器, 值)`。
    pub extra_results: Vec<(X64Register, u128)>,
    /// 内存槽位。
...
pub struct X64DecodeFixture {
    ...
    pub code: X64Code,
    /// 输入压缩字所在的寄存器。
    pub word_register: X64Register,
    /// 解码结果写出的寄存器。
    pub result_register: X64Register,
    /// 压缩字与期望解码结果：`None` 表示空字，非空字只有 `Some` 才是合法解码。
...
impl X64Harness {
    ...
    pub fn report(&self) -> X64HarnessReport {
        let cage_sites = cage_decode_sites(self.target);
        let code_len = u32::try_from(self.switch.bytes.len()).unwrap_or(u32::MAX);
        X64HarnessReport {
...
fn cage_decode_sites(target: TargetName) -> u32 {
    ...
    let Some(lir) = compilation.lir.as_ref() else {
        return 0;
    };
    let mut sites = 0_u32;
...
fn decode_fixture(baseline: CpuBaseline) -> Result<X64DecodeFixture, X64HarnessError> {
    let reference = crate::runtime::DecodeReferenceFixture::new().map_err(unavailable)?;
    let code = decode_code(baseline)?;
    if code
        .relocations
        .iter()
        .any(|relocation| !matches!(relocation.target, "cage-control" | "cold"))
    {
        return Err(X64HarnessError::DecodeUnavailable {
            detail: "解码片段只允许 cage-control 与冷边重定位".to_owned(),
        });
    }
    Ok(X64DecodeFixture {
        ...
        control_fields: CAGE_CONTROL_FIELDS
            ...
            .collect(),
        word_register: X64Register::Gpr(Gpr::Rax.code()),
        result_register: X64Register::Gpr(Gpr::Rbx.code()),
        code,
        words: reference.words,
        decodes_field: control_field("decodes"),
        rejections_field: control_field("rejections"),
        decodes: reference.decodes,
        rejections: reference.rejections,
    })
}

fn unavailable(error: impl std::fmt::Display) -> X64HarnessError {
...
fn control_field(name: &str) -> (u32, u32) {
    CageControlRecord::field(name).expect("字段表登记全部控制记录字段")
}

/// 解码片段：`DecodeCompressedRef` 的 lowering 结果与 `cage-control` 重定位。
fn decode_code(baseline: CpuBaseline) -> Result<X64Code, X64HarnessError> {
...
///
/// 返回值是 `(片段, restore 入口偏移, 是否与运行时逐字节相同)`。
fn switch_fragment() -> Result<(X64Code, u32, bool), X64HarnessError> {
    ...
    let mut save = Sequence::new();
    // lea rax, [rsp + 8]；mov [rdi + rsp_off], rax：保存调用者 return PC。
    push(
        &mut save,
        "lea",
    ...
    push(
        ...
        Access::Write,
        vec![
            memory(Reg::Gpr(Gpr::Rdi), rsp),
            register(Reg::Gpr(Gpr::Rax)),
        ],
    )?;
    // mov rax, [rsp]：读回 return PC，再写进 rip 字段。
    push(
        &mut save,
        "mov",
        &[OperandKind::Rm64, OperandKind::R64],
    ...
    for (field, source) in [
        ...
        push(
            ...
            vec![
                ...
                register(Reg::Gpr(source)),
            ],
        )?;
    }
    let saved = assemble(&save).map_err(|error| X64HarnessError::Encoding {
        case: "switch-save".to_owned(),
        detail: error.to_string(),
    })?;
    let restore_offset =
        u32::try_from(saved.bytes.len()).map_err(|_| X64HarnessError::MissingForm {
            mnemonic: "switch restore offset".to_owned(),
        })?;
    let mut sequence = save;
    // mov r14, rdx；mov r15, rcx：内部调用约定的固定寄存器（r/m 位置是目标）。
    for (source, target) in [(Gpr::Rdx, Gpr::R14), (Gpr::Rcx, Gpr::R15)] {
        push(
            &mut sequence,
            "mov",
            &[OperandKind::Rm64, OperandKind::R64],
            Access::Write,
            vec![register(Reg::Gpr(target)), register(Reg::Gpr(source))],
        )?;
    }
    for (field, target) in [
        (rbx, Gpr::Rbx),
        ...
        push(
            ...
            Access::Read,
            vec![
                memory(Reg::Gpr(Gpr::Rsi), field),
                register(Reg::Gpr(target)),
            ],
        )?;
    }
    push(
...
fn disp(offset: usize) -> Result<i32, X64HarnessError> {
    i32::try_from(offset).map_err(|_| X64HarnessError::MissingForm {
        mnemonic: "context field offset".to_owned(),
    })
}
...
fn push(
    ...
    let Some(form) = table::form_id_with_access(mnemonic, kinds, first) else {
